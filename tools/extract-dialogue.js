...
function main() {
  ...
  readme.push('この Markdown と同じ `TABLE_MANIFEST` を再利用しているため、対象テーブルの一覧はここに載っている表と一致する。');
  readme.push('');
  readme.push('コマンドを直接叩く代わりに、`セリフ編集/` 直下の `1_エクセルに書き出し.bat` / `2_変更内容を確認.bat` / `3_ゲームに反映.bat` をダブルクリックしても同じ操作ができる(詳細は同フォルダの `README.txt`)。');
  readme.push('');
  readme.push('### 分割軸(2026-07-25 改訂): カテゴリ別 → 人格(archetype×personality)別');
  readme.push('');
  readme.push('実際の編集は「その属性(archetype)と性格(personality)の一人を想像して、その人の全セリフを通しで直していく」という単位で行われる。');
  readme.push('そのため `セリフ編集/` はカテゴリ別20ファイルではなく、以下のフォルダ構成になっている:');
  readme.push('');
  readme.push('| フォルダ | 内容 |');
  readme.push('|---|---|');
  ...
  readme.push('');
  readme.push('```');
  readme.push('# 1. 書き出し(セリフ編集/ 配下を丸ごと再生成する。カテゴリ指定は廃止)');
  readme.push('node tools/dialogue-workbook.js export');
  readme.push('');
  readme.push('# 2. セリフ編集/ 配下の xlsx を Excel で開き、「改訂」列にだけ書き込む');
  readme.push('#    (他の列は書き戻しに使う参照情報なので触らない。ID列は特に編集不可。');
  readme.push('#     直したいキャラのタイプが分からなければ まず _キャラ対応表.xlsx を開く)');
  readme.push('');
  readme.push('# 3. 差分プレビュー(何も書き込まない。ファイル名だけでも指定できる)');
  readme.push('node tools/dialogue-workbook.js apply "セリフ編集/キャラタイプ別/丁寧×真面目.xlsx" --dry-run');
  readme.push('');
  readme.push('# 4. 実際に src/*.js へ書き戻す(引数省略で セリフ編集/ 配下の全 .xlsx が対象。フォルダをまたいで再帰的に走査する)');
  readme.push('node tools/dialogue-workbook.js apply "セリフ編集/キャラタイプ別/丁寧×真面目.xlsx"');
  ...
  readme.push('| archetype / personality | キー名から推定した口調/性格(推定できない場合は空欄。ベストエフォート。この束に来る行は定義上どちらも未解決/部分解決) |');
  ...
  readme.push('- ID はどのファイル/フォルダに置かれているかに依存しない(ソース上のパスそのもの)。ファイルを移動しても書き戻しには影響しない。');
  readme.push('');
  readme.push('### 既知の制約');
  readme.push('');
  readme.push('- **`EVENT_LINES_BY_KEY`(その他セリフ/12-選手経歴イベント.xlsx 内)は書き戻せない**: このテーブルは `draftJoin: EVENT_DRAFT_JOIN_LINES` のように他の既存テーブルへの**参照**だけで構成されたエイリアスであり、ソーステキスト上には文字列リテラルが存在しない。中身を書き直したい場合は、参照先の元テーブル(`EVENT_DRAFT_JOIN_LINES` など、いずれも同じカテゴリに個別収録されている)を編集すること。apply はこの行を自動的にスキップ+警告する。');
