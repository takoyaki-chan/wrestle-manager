  ...
  while (completed < seasons && iter < MAX_ITER) {
    ...
    try {
      ...
      if (!G.offSeason && G.week === 1 && G.season > 1) {
        ...
        // ── ORG ROSTER TRACKING ──
        if (!stats.orgRosters) stats.orgRosters = { org_s: [], org_a: [], org_b: [] };
        ['org_s', 'org_a', 'org_b'].forEach(k => {
          const r = G.aiOrgs && G.aiOrgs[k] && G.aiOrgs[k].roster ? G.aiOrgs[k].roster.length : 0;
          stats.orgRosters[k].push(r);
        });
        if (!stats.dormantHistory) stats.dormantHistory = [];
        stats.dormantHistory.push((G.dormantPool || []).length);
        if (!stats.faHistory) stats.faHistory = [];
        stats.faHistory.push((G.freeAgents || []).length);
...
// ── ORG ROSTER TRACKING REPORT ──
if (result.stats && result.stats.orgRosters) {
  const or = result.stats.orgRosters;
  const summarize = (arr) => {
    ...
    return {
      ...
      under10: arr.filter(v => v < 10).length,
      under12: arr.filter(v => v < 12).length,
    };
  };
  console.log('--------------------------------------');
  console.log('AI団体ロスター推移（シーズン末）');
  ['org_s', 'org_a', 'org_b'].forEach(k => {
    const s = summarize(or[k]);
    if (!s) return;
    console.log(`  ${k}: avg=${s.avg} med=${s.med} std=${s.std} min=${s.min} max=${s.max} p10=${s.p10} p90=${s.p90} <10:${s.under10} <12:${s.under12}`);
  });
  // 直近20シーズンのorg_s推移を表示
  const recent = or.org_s.slice(-30);
  console.log(`  org_s 直近${recent.length}: [${recent.join(',')}]`);
  // dormant/FA も併記
  if (result.stats.dormantHistory) {
    const ds = summarize(result.stats.dormantHistory);
    console.log(`  dormantPool: avg=${ds.avg} min=${ds.min} max=${ds.max}`);
  }
  if (result.stats.faHistory) {
    const fa = summarize(result.stats.faHistory);
    console.log(`  freeAgents : avg=${fa.avg} min=${fa.min} max=${fa.max}`);
  }
}
