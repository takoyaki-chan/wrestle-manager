...
// ===========================================================================
// 4. buildOrgSummaryCard — database「団体比較」画面のVSカード(ui-render.js)
//    是正対象: 従来は名前+ティアバッジのみでエンブレムが一切無かった(specs/newspaper-and-orgcompare-spec-v2.0.md
//    §np-org-summary は元々「name + tier badge + tags + stats grid + 王者表示」とだけ定義していた)。
// ===========================================================================
(function orgSummaryCardSuite() {
  const build = new Function(`${uiRenderFn('buildOrgSummaryCard')} return { buildOrgSummaryCard };`);
  const built = build();

  function callCard(emblemHtml) {
    return built.buildOrgSummaryCard(
      '紅蓮プロレス', 'サブタイトル', 'player',
      '<div class="db-cmp-tier player">プレイヤー</div>',
      ['タグ1'], 12, 55, { ace: 300 }, '不在', '',
      '', emblemHtml
    );
  }

  section('buildOrgSummaryCard: renders the passed emblem markup inside a dedicated title wrapper (unvariant 2)', () => {
    let html;
    assert.doesNotThrow(() => { html = callCard('<img src="image/org/org-player-0.png" alt="">'); }, '例外を投げない(不変条件1)');
    assert.ok(html.includes('db-cmp-org-summary-title'), 'エンブレム+名前をまとめる専用ラッパーが出る');
    assert.ok(html.includes('image/org/org-player-0.png'), '実エンブレム画像が出る');
    assert.ok(html.includes('紅蓮プロレス'), '団体名は従来どおり出る(機能を削っていない)');
    assert.ok(html.includes('db-cmp-tier player'), 'ティアバッジも従来どおり出る');
  });

  section('buildOrgSummaryCard: does not throw and still renders the title wrapper when no emblem markup is given (unvariant 4)', () => {
    let html;
    assert.doesNotThrow(() => { html = callCard(''); }, '空文字でも例外なし');
    assert.ok(html.includes('db-cmp-org-summary-title'), 'エンブレムが無くてもラッパー自体は出る(壊れない)');
    assert.doesNotThrow(() => built.buildOrgSummaryCard('X', 'Y', 'rival', '', [], 0, 0, { ace: 0 }, '不在', ''),
      'emblemHtml省略(デフォルト引数)でも例外なし');
  });

  // 呼び出し側(_renderDbOrgCompare)が実エンブレムを渡していることのソース確認
  // (Engine.database.getOrgCompareAnalysis 等の依存が重く全体を実行するのは非現実的なため、
  //  test/show-result-summary-theme-test.js が確立した「ソース文字列で結線を確認する」方式を踏襲)
  section('_renderDbOrgCompare wires real org emblems (26px) into both summary cards', () => {
    assert.ok(uiRender.includes("_npOrgEmblem(G, 'player', 26)"), 'プレイヤーカードへ実エンブレムを渡している');
    assert.ok(uiRender.includes('_npOrgEmblem(G, _dbCompareTarget, 26)'), '比較相手カードへ実エンブレムを渡している');
  });
})();
...
// ===========================================================================
// 5. showPPVVSDetail(renderSide) — PPV VS比較ポップアップ(ui-common.js)
// ===========================================================================
(function ppvVsDetailSuite() {
  const build = new Function(
    'App', 'Audio', 'Engine', 'getPortraitUrl', 'portraitImg', 'RIVAL_ORGS', 'document', 'G',
    `${uiFn('escHtml')}
     ${uiFn('orgIconHtml')}
     ${uiFn('showPPVVSDetail')}
     return { showPPVVSDetail };`
  );

  function fighter(id, name, orgId, orgName, extra) {
    return Object.assign({
      id, name, style: 'Grappler', _ppvOrgId: orgId, _ppvOrgName: orgName,
      pw: 60, sp: 60, te: 60, st: 60, mn: 60, popularity: 50, traits: [],
    }, extra || {});
  }
  ...
  function makeBundle(opts) {
    ...
    const documentStub = {
      getElementById: (id) => (id === 'fighterPopupBox' ? box : id === 'fighterPopupOverlay' ? overlay : null),
    };
    const EngineStub = opts.Engine || {
      util: {
        ov: () => 70,
        dispPop: (p) => String(p || 0),
        getPlayerOrgIconPath: () => 'image/org/org-player-0.png',
        getOrgIconPath: (state, orgId) => (orgId === 'org_a' ? 'image/org/org-a-0.png' : orgId === 'org_b' ? 'image/org/org-b-0.png' : ''),
      },
      title: { getRivalryLevel: () => null },
    };
    const getPortraitUrlStub = opts.getPortraitUrl || ((id) => `image/face/${id}.png`);
    const portraitImgStub = opts.portraitImg || (() => '');
    const RIVAL_ORGS_STUB = opts.RIVAL_ORGS || [
      { id: 'org_a', color: '#6c5ce7' }, { id: 'org_b', color: '#00b894' },
    ];
    const AudioStub = opts.Audio || { play() {} };
    const GStub = opts.G || { rivalries: {} };
    const AppStub = { _ppvPreview: opts.ppvPreview };
    const built = build(AppStub, AudioStub, EngineStub, getPortraitUrlStub, portraitImgStub, RIVAL_ORGS_STUB, documentStub, GStub);
    return { built, box };
  }

  section('cross-org match: shows a real emblem for both sides (unvariant 2, 3)', () => {
    const left = fighter(1, '風間サキ', 'player', 'プレイヤー団体');
    const right = fighter(2, '結城ミナ', 'org_a', 'ライバル団体');
    const { built, box } = makeBundle({ ppvPreview: { card: [{ left, right, isSummit: false }] } });
    assert.doesNotThrow(() => built.showPPVVSDetail(0), '例外を投げない(不変条件1)');
    const html = box.innerHTML;
    assert.ok(html.includes('image/org/org-player-0.png'), '自陣の実エンブレムが出る');
    assert.ok(html.includes('image/org/org-a-0.png'), '敵陣の実エンブレムが出る');
    assert.ok(html.includes('プレイヤー団体') && html.includes('ライバル団体'), '両者の団体名が出る');
  });

  section('same-org match: omits the emblem on both sides but keeps the org name text (badge display condition)', () => {
    const left = fighter(1, '白鳥レイ', 'org_b', '団体B');
    const right = fighter(2, '東城カレン', 'org_b', '団体B');
    const { built, box } = makeBundle({ ppvPreview: { card: [{ left, right, isSummit: false }] } });
    built.showPPVVSDetail(0);
    const html = box.innerHTML;
    assert.ok(!html.includes('image/org/org-b-0.png'), '自団体内の対戦ではエンブレムを出さない(出す条件: 他団体が絡む試合のみ)');
    assert.ok(html.includes('団体B'), '団体名のテキスト自体は変わらず出る(配置は変えずバッジだけ省く)');
  });

  section('escapes a hostile org name and does not throw for a missing match index (unvariant 4, 5)', () => {
    const left = fighter(1, 'A', 'org_a', '<script>alert(1)</script>');
    const right = fighter(2, 'B', 'org_b', '団体B');
    const { built, box } = makeBundle({ ppvPreview: { card: [{ left, right, isSummit: false }] } });
    built.showPPVVSDetail(0);
    ...
    const { built: built2 } = makeBundle({ ppvPreview: { card: [] } });
    assert.doesNotThrow(() => built2.showPPVVSDetail(0), '存在しないmatchIdxでも例外なし');
  });
})();

// ===========================================================================
// 6. _renderB3MatchPreview — 挑戦状(B3)対戦プレビュー(ui-common.js)
//    このイベントはplayer対特定のAI団体で必ず他団体戦になるため、常にエンブレムを出す。
// ===========================================================================
(function b3MatchPreviewSuite() {
  const build = new Function(
    'document', 'RIVAL_ORGS', 'Engine', 'G', 'getStandUrl', 'pickDialogueLine', 'PPV_OPPONENT_LINES',
    `${uiFn('escHtml')}
     ${uiFn('orgIconHtml')}
     ${uiFn('_rgba')}
     ${uiFn('_lightenColor')}
     ${uiFn('_warStatRow')}
     ${uiFn('_renderB3MatchPreview')}
     return { _renderB3MatchPreview };`
  );
  ...
  function makeBundle(opts) {
    ...
    const RIVAL_ORGS_STUB = opts.RIVAL_ORGS || [{ id: 'org_a', color: '#6c5ce7', emoji: '💫' }];
    const EngineStub = opts.Engine || {
      util: {
        ov: () => 70,
        getPlayerOrgIconPath: () => 'image/org/org-player-0.png',
    ...
    const getStandUrlStub = opts.getStandUrl || ((id) => `image/stand/${id}.png`);
    const pickDialogueLineStub = opts.pickDialogueLine || (() => '');
    const built = build(documentStub, RIVAL_ORGS_STUB, EngineStub, GStub, getStandUrlStub, pickDialogueLineStub, {});
    return { built, box };
  }

  const playerFighter = { id: 1, name: '風間サキ', pw: 60, sp: 60, te: 60, st: 60, mn: 60, traits: [] };
  const challenger = { id: 2, name: '黒羽ミナ', pw: 55, sp: 55, te: 55, st: 55, mn: 55, traits: [] };

  section('renders a real emblem for both the player org and the challenging org (unvariant 2, 3)', () => {
    const { built, box } = makeBundle({});
    assert.doesNotThrow(() => built._renderB3MatchPreview({ orgName: '鬼道場', orgId: 'org_a' }, playerFighter, challenger),
      '例外を投げない(不変条件1)');
    const html = box.innerHTML;
    assert.ok(html.includes('image/org/org-player-0.png'), '自陣(プレイヤー)の実エンブレムが出る');
    assert.ok(html.includes('image/org/org-a-0.png'), '相手団体の実エンブレムが出る');
    assert.ok(html.includes('プレイヤー団体') && html.includes('鬼道場'), '両陣営の団体名が出る');
  });

  section('escapes a hostile org name and does not throw when the org is unresolvable (unvariant 4, 5)', () => {
    const { built, box } = makeBundle({});
    assert.doesNotThrow(() => built._renderB3MatchPreview({ orgName: '<script>alert(1)</script>', orgId: 'org_z' }, playerFighter, challenger),
      '未知の団体IDでも例外なし');
    const html = box.innerHTML;
    assert.ok(!html.includes('<script>alert(1)</script>'), '団体名は生のまま出力されない');
    assert.ok(html.includes('&lt;script&gt;'), 'escHtml()でエスケープされる');
