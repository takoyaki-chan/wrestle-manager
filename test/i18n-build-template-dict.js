...
//    6. プレースホルダ直後の可算名詞複数形(P7-10で新設・P7-13でexit 1化): 同 §3-4 規則23。
//       `{count} times` のように数値PHの直後に可算名詞の複数形を置くと、充填値が1のとき
//       単複が食い違う。コロン列挙型・ハイフン限定用法(規則24)・単位を持たない形へ
//       書き直して逃がす。名前・団体名PH+wins/reignsの三人称単数動詞は誤検知として除外。
//
//  ■ 使い方
...
function main() {
  ...
  ledger.forEach((entry, idx) => {
    ...
    // 6. プレースホルダ直後の可算名詞複数形(§3-4 規則23。P7-13でexit 1化)
