...
const PATCHES = {
  ...
  RIVALRY_MATCH_REACTION: {
    winnerLines: {
      normal: { polite: ['まだ終わっていません。次も…必ず勝ちます'] },
      bold: { delinquent: ['通過点だ。次もぶっ倒してやる'], seductive: ['まだ先があるのよ。…楽しみにしてなさい'], polite: ['この勝利は通過点です。まだ先がありますから'] },
      quiet: { seductive: ['………まだよ'], polite: ['………まだ…終わりではありません'] },
      easygoing: { seductive: ['ひとつ勝ち。ふふ…でもまだこれからよ'], polite: ['一つ勝たせていただきました！ でもまだまだこれからです'], ojousama: ['一つ勝ちですわ！ でもまだまだこれからですの'] },
      earnest: { seductive: ['勝ったわ。でもこの因縁…まだ続くの'], ojousama: ['勝たせていただきましたわ。この因縁はまだ続きますの'] }
    },
    loserLines: {
      normal: { polite: ['次は負けません。この借りは…必ず返します'] },
      bold: { delinquent: ['この借り、絶対返すからな'], seductive: ['この借り…必ず返すわ。覚悟しておきなさい'], polite: ['この負けは認めません。必ず借りを返します'] },
      quiet: { seductive: ['………次は、ないわよ'], polite: ['………次は…負けません'] },
      easygoing: { seductive: ['あら、負けちゃった。…でも次は覚悟しておいてね'], polite: ['負けてしまいました。でも次は必ずやり返します！'], ojousama: ['負けてしまいましたわ。…でも次はやり返しますの'] },
      earnest: { seductive: ['負けたわ。でも…諦めないの。必ず返すから'], ojousama: ['負けましたわ。でも諦めませんの。必ず借りを返しますわ'] }
    }
  },

  BITTER_RESOLUTION_LINES: {
    winner: {
      normal: { polite: ['……終わりました。もう二度と…関わりたくありません'] },
      bold: { seductive: ['もう終わりよ。二度と関わりたくないの'], polite: ['終わりです。もう振り返りません'] },
      quiet: { seductive: ['………もう…終わりよ'], polite: ['………もう…終わりです'] },
      easygoing: { seductive: ['…ふぅ。やっと終わったわね。もう勘弁して'], polite: ['……やっと終わりました。もう勘弁してください'], ojousama: ['……やっと終わりましたわ。もうご勘弁くださいまし'] },
      earnest: { seductive: ['決着がついたわ。…もう、十分よ'], ojousama: ['決着がつきましたわ。…もう、十分ですの'] }
    },
    loser: {
      normal: { polite: ['……認めません。こんな結末は…認められません'] },
      bold: { seductive: ['認めないわ…この借りは必ず返すから'], polite: ['認めません…この借りは必ず返します'] },
      quiet: { seductive: ['………認めない…わ'], polite: ['………認め…ません'] },
      easygoing: { seductive: ['…参ったわね。でも…忘れないわよ'], polite: ['……参りました。でも…忘れませんから'], ojousama: ['……参りましたわ。でも…忘れませんことよ'] },
      earnest: { seductive: ['認めないわ…こんな結末は…認められないの'], ojousama: ['認めませんわ。こんな結末は…認められませんの'] }
    }
  },
  ...
  // ─── Session F (flat sources only): イベント系 ───

  PPV_OPPONENT_LINES: {
    normal: {
      cool: ['……受けて立つ。全力だ'],
      polite: ['全力でお相手させていただきます。よろしくお願いします']
    },
    bold: {
      polite: ['全力で参ります。覚悟してください']
    },
    quiet: {
      seductive: ['………行くわよ']
    },
    easygoing: {
      polite: ['全力で行きます！ よろしくお願いします！'],
  ...
  GLIMPSE_HOTSTREAK_END_LINES: {
    normal: {
      polite: ['連勝が途切れてしまいました。…でも、ここからまた頑張ります']
    },
    bold: {
      polite: ['連勝は終わりましたが、まだまだこれからです']
    },
    quiet: {
      seductive: ['………終わったわね。…でもまだよ']
    },
    easygoing: {
      polite: ['連勝ストップかあ…でもまた頑張ります！'],
      ojousama: ['連勝が止まりましたわ…でもまた頑張りますの']
    }
  },
...
const FLAT_SOURCES = new Set([
  ...
  'ENDING_LINES',
  'PPV_OPPONENT_LINES', 'GLIMPSE_HOTSTREAK_END_LINES'
]);
...
function doInsert(text, persMatchPos, archetype, lines) {
  ...
  if (composedMatch) {
    insertPos = persMatchPos + composedMatch.index;
    indent = composedMatch[2];
  } else {
    ...
    indent = closeMatch[2] + '  ';
    insertPos = persMatchPos + closeMatch.index;
  }

  // Ensure trailing comma before insertion point
  let prefix = '';
  const beforeInsert = text.slice(Math.max(0, insertPos - 5), insertPos);
  if (/\]\s*$/.test(beforeInsert) && !/,\s*$/.test(beforeInsert)) {
    // Find the last ] before insertPos and add comma after it
    const lastBracket = text.lastIndexOf(']', insertPos - 1);
    if (lastBracket >= 0 && lastBracket > insertPos - 20) {
      const patched = text.slice(0, lastBracket + 1) + ',' + text.slice(lastBracket + 1);
      insertPos += 1; // shift for added comma
      const newEntry = buildEntry(indent, archetype, lines);
      return { changed: true, text: patched.slice(0, insertPos) + newEntry + patched.slice(insertPos) };
    }
  }
...
if (DRY_RUN) {
  ...
  fs.writeFileSync(DATA_PATH, patched, 'utf8');
...
try {
  ...
  if (!DRY_RUN) {
    // Restore original
    fs.writeFileSync(DATA_PATH, original, 'utf8');
