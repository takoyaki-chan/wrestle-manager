...
// MQ再設計P4 §5.2: 大ニュース(一面ジャック+週頭通知)判定集合。
// 現在はMQ記録更新2種のみ。P5で hotProspectDebut / fatedRivals / topChampionInjury を追加予定。
const BIG_NEWS_TYPES = new Set(['mqAllTimeRecord', 'mqTagRecord']);

// MQ再設計P4 §5.3: 週頭ポップアップの号外リード文言（bignews-article-drafts-v1.0.md 正本）。
// {mq} のみ変数展開（記事本文とは独立に、そのつどランダム1本を選ぶ）。
