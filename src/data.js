...
const NOTIF_EVENT_TEXTS = {
  N1: [
    '💪 {name}が自主トレで手応えを掴んだ',
    '🌟 {name}の努力が実を結びつつある',
    '✨ {name}が練習で目を引くプレーを見せた',
    '📈 {name}の動きが明らかに良くなっている',
  ],
  N2: [
    '🤝 {name}と{name2}が練習後に話し込んでいた',
    '💬 {name}が{name2}に技のコツを教えていた',
    '🌸 {name}と{name2}の仲が深まってきた',
    '👊 {name}と{name2}が激しく語り合っていた',
  ],
  N3: [
    '😓 {name}が少し疲れ気味のようだ…',
    '🌡️ {name}のコンディションが優れない',
    '💤 {name}の練習に覇気がない日があった',
    '😔 {name}が練習量を落としているようだ',
  ],
  N4: [
    '📣 ファンから{name}への応援の声が増えている！',
    '🎉 SNSで{name}が話題になっている！',
    '💖 {name}目当てのファンが増えてきた！',
    '🔥 {name}の人気が上り調子だ！',
  ],
  N5_warning: [
    '😶 {name}が最近どことなく元気がないようだ…',
    '💭 {name}の様子が少し気になる',
    '🤔 {name}が浮かない顔をしていた',
    '😑 {name}が練習中に何かを考えているようだった',
  ],
  N5_low: [
    '😤 {name}が最近不満そうにしている…',
    '😟 {name}から笑顔が消えてきた気がする',
...
const CARE_ACTIONS = {
  ...
  party: {
    ...
    effects: { trust_all: 2, morale: 5 },
  },
  camp: {
    id: 'camp', label: '合宿', emoji: '⛺', cost: 320, category: 'team',
    desc: '全員の成長+中（2週間集中）、信頼度+2',
    effects: { growth_all: { weeks: 2, mult: 1.5 }, trust_all: 2 },
  },
};
