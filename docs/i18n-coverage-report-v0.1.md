...
| ~~ui-render.js~~ **✅P7-31で全数仕分け済み** | 104 | 1,396 | `追い込みを続けると体が重くなり、同じ練習でも身につきにくくなる。休ませると戻る。`(→**育成画面😮‍💨の常設ティップス**。消費点は正しく`t()`だが台帳に無かった)、`常に落ち着いた佇まいが格を生む`(→**`_aceFlavorByPersona` は1度も呼ばれない死骸**)、HTML塊の断片多数(→序章オーバーレイ4幕・派閥クロニクル・王座奪還/空位バナー等)。**79件の内訳: 訳した39 / 論理比較で除外5 / HTMLコメント1 / 死骸34**。詳細は specs/i18n-runtime-spec-v1.0.md §39 |
