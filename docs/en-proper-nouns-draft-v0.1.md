...
- **技名 160種** — **本ドラフトでは扱わない。** 量が多く(`specs/技テーブル_全160技_v3_5.md`)、かつ「実在技の英語正式名(Powerbomb / Lariat / German Suplex)」「創作技のリングネーム的命名」「フィニッシャーの固有名」で判断軸が3つに割れるため、**別ドラフト(次工程)** に分ける
  - **✅完了(2026-09-04 P7-5)**: 別ドラフト `docs/en-move-names-draft-v0.1.md`(実データは242件)でKeisuke裁定済み。**技名も本ドラフトと同じ「名前辞書」の住人**として実装した — `i18n/names-ledger.json` の `moves` 節 → `src/lang-en-names.js` の `addMoves`/`addMoveShorts` → `WM_I18N.mv()`/`mvShort()`。ただし `pn()` が引く `names` とは**別領域**にしてある(技名の日本語は効果音判定・解説文選択・セーブ値 `finMove` の安定キーとして生き続けるため)。詳細は `specs/i18n-runtime-spec-v1.0.md` §23
