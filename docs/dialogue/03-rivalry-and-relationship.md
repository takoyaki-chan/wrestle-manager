...
- 出典: `src/data.js`
- コード内コメント: 週次ティッカーテキスト
- 本数: 65

### bestFriends[]

- `WEEKLY_STORY_TICKER.bestFriends[1]`: {nameA}と{nameB}が練習後に並んで帰る姿が目撃された
- `WEEKLY_STORY_TICKER.bestFriends[2]`: {nameA}と{nameB}がトレーニングで息の合った動きを見せている
- `WEEKLY_STORY_TICKER.bestFriends[3]`: {nameA}が{nameB}の体調を気遣っている姿があった

### hostileEnemy[]

- `WEEKLY_STORY_TICKER.hostileEnemy[1]`: {nameA}と{nameB}の間に張り詰めた空気が漂っている
- `WEEKLY_STORY_TICKER.hostileEnemy[2]`: {nameA}と{nameB}が控室で目も合わせない
- `WEEKLY_STORY_TICKER.hostileEnemy[3]`: ロッカールームの一角で{nameA}と{nameB}の冷たい空気が流れている

### clash[]

- `WEEKLY_STORY_TICKER.clash[1]`: {nameA}と{nameB}が練習中に激しく言い合いになった
- `WEEKLY_STORY_TICKER.clash[2]`: {nameA}と{nameB}の口論が控室に響いた
- `WEEKLY_STORY_TICKER.clash[3]`: {nameA}が{nameB}に食ってかかり、スタッフが止めに入った
- `WEEKLY_STORY_TICKER.clash[4]`: {nameA}と{nameB}の間で一触即発の空気が走った
- `WEEKLY_STORY_TICKER.clash[5]`: {nameA}と{nameB}が掴み合いになりかけ、周囲が慌てて止めた

### goodRivalZone[]

- `WEEKLY_STORY_TICKER.goodRivalZone[1]`: {nameA}が{nameB}の試合映像を繰り返し見ている
- `WEEKLY_STORY_TICKER.goodRivalZone[2]`: {nameA}と{nameB}が練習後に言葉を交わし、互いに頷いていた
- `WEEKLY_STORY_TICKER.goodRivalZone[3]`: {nameA}は{nameB}の試合結果を真っ先に確認している

### unrequitedBond[]

- `WEEKLY_STORY_TICKER.unrequitedBond[1]`: {nameA}が{nameB}に話しかけるも、素っ気ない反応だった
- `WEEKLY_STORY_TICKER.unrequitedBond[2]`: {nameA}は{nameB}のことを親友だと思っている——{nameB}がどう思っているかは別として
- `WEEKLY_STORY_TICKER.unrequitedBond[3]`: {nameA}が{nameB}を誘うが、{nameB}は別の用事があると去っていった

### onesidedHostility[]

- `WEEKLY_STORY_TICKER.onesidedHostility[1]`: {nameA}が{nameB}を睨みつけている。{nameB}は気づいていない
- `WEEKLY_STORY_TICKER.onesidedHostility[2]`: {nameA}は{nameB}の名前を聞くたびに表情を歪めている
- `WEEKLY_STORY_TICKER.onesidedHostility[3]`: {nameA}の練習が日に日に荒くなっている——{nameB}への執念が滲む

### temperatureDiff[]

- `WEEKLY_STORY_TICKER.temperatureDiff[1]`: {nameA}が{nameB}との距離に少し戸惑っている様子だ
- `WEEKLY_STORY_TICKER.temperatureDiff[2]`: {nameA}は{nameB}との関係をもっと深めたいようだが……

### crossAsymmetry[]

- `WEEKLY_STORY_TICKER.crossAsymmetry[1]`: {nameA}は{nameB}を目の敵にしている。{nameB}は{nameA}のことを仲間だと思っているのに
- `WEEKLY_STORY_TICKER.crossAsymmetry[2]`: {nameB}が笑顔で話しかけた{nameA}は、拳を握りしめて背を向けた

### highRivalryAwareness[]

- `WEEKLY_STORY_TICKER.highRivalryAwareness[1]`: {nameA}は客席から{nameB}の試合を食い入るように見つめていた
- `WEEKLY_STORY_TICKER.highRivalryAwareness[2]`: {nameA}は{nameB}の試合結果を聞いて、ゆっくりとテーピングを巻き直した
- `WEEKLY_STORY_TICKER.highRivalryAwareness[3]`: {nameA}が{nameB}の勝利を知り、静かに拳を握った
- `WEEKLY_STORY_TICKER.highRivalryAwareness[4]`: 花道の奥から{nameA}が{nameB}の試合を見ていた。その目は揺れていない

### goodRivalTicker[]

- `WEEKLY_STORY_TICKER.goodRivalTicker[1]`: {nameA}と{nameB}が練習後に静かに言葉を交わしていた
- `WEEKLY_STORY_TICKER.goodRivalTicker[2]`: {nameA}と{nameB}——好敵手の二人が並んでストレッチをしている
- `WEEKLY_STORY_TICKER.goodRivalTicker[3]`: {nameA}は{nameB}の試合を見て、小さく微笑んだ

### bitterRivalTicker[]

- `WEEKLY_STORY_TICKER.bitterRivalTicker[1]`: {nameA}と{nameB}が控室ですれ違い、空気が凍りついた
- `WEEKLY_STORY_TICKER.bitterRivalTicker[2]`: {nameA}の表情が一瞬歪んだ——{nameB}の名前を聞いただけで
- `WEEKLY_STORY_TICKER.bitterRivalTicker[3]`: {nameA}と{nameB}が同じ廊下を歩くことを、誰もが避けさせている

### trustWarning[]

- `WEEKLY_STORY_TICKER.trustWarning[1]`: {name}の表情が最近曇っている
- `WEEKLY_STORY_TICKER.trustWarning[2]`: {name}の練習への集中力が落ちてきている
