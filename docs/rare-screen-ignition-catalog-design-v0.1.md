...
|---|---|---|---|---|
| R1 | **天頂戦通年**(W42ミニイベント→W43エントリー→W48開催15試合→優勝演出→**初代統一王座戴冠**→季末→新シーズン) | 4季に1回 | season4・W41到達までheadless進行(加工不要。ppvUnlocked必須assert) | ✅ 実装済み |
| R2 | **ゲームオーバー**(資金危機突入→即死判定→解散セレモニー5スライド→GAME OVER画面) | 踏んだら終わり | funds を -1600 に加工(1tick目で危機突入、2tick目で即死。突入バナーも通る) | ✅ 実装済み |
| R3 | 果たし状(相手発・迎撃画面)+遠征2拍リザルト(task-87/95の新画面) | 確率イベント | 挑戦系コンテナ予約を加工(challenge-request-spec §7の排他と整合させる) | 次バッチ |
| R4 | 統一王座の防衛サイクル(挑戦表明/こちらの番/遠征/迎撃/返還式) | 王者存在が前提 | R1後の状態(season5+)から。unifiedTitle保持者を起点に | 次バッチ |
| R5 | 派閥開戦セレモニー+開戦後UI(task-86) | 敵対度の蓄積待ち | 2派閥の敵対度をしきい値直前に加工 | 次バッチ |
