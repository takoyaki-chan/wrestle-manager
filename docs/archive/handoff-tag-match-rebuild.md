...
> 作成日: 2026-04-17
> 最終更新: 2026-04-17 (T2 バグ修正 + B2 OK確認 → 全項目クローズ、archive 入り)
> **ステータス: CLOSED** — 残タスク T1/T3/T4 は roadmap に移管済み
...
**2026-04-17 セッション後半 (d3f5937 〜 5e65aff) で完了した項目:**
...
- ✅ (副次) ピン seq 中のネタバレ抑制 (「決着！」narration と「★」ログを seq 完了まで保留) (5e65aff)
- ✅ (副次) 最終 step 連打で endMatch 発火 → 結果画面を飛ばすバグ修正 (5e65aff)

**2026-04-17 セッション末 (T2 バグ修正) で完了した項目:**
- ✅ **T2** 開幕HP 83% 表示バグ修正: playback 側 mk 式 `85 + st*1.20` → `70 + st*1.00` (TAG_MATCH_CONFIG と一致) (`src/tag-battle-main.js:69`)

**残り (別日セッション / roadmap にも記載済み):**
- **B2** friendlyFire 体感調整 (実プレイで違和感あれば数値見直し) — 判断保留
- **F2** (保留) F1 修正後 `touchPause` 700ms が冗長化していないか検証
- **T1** 試合が50ターン級に長引く問題 — ピン成立率 or 疲労蓄積の再調整。E2 系の続き
- **T3** ダブルチーム演出のテキスト詳述 — 何をやっているか narration で分かるように
- **T4** 試合結果画面で各選手の一言台詞 — 決着画面にキャラごとのコメント追加 (新機能)
...
| B2 | Balance | 低 | ✅完 | friendlyFire 誤爆ダメージ (2-4＋MHP50%フロア) の体感 | — (実プレイOK) |
...
- `5e65aff` — ピン seq 中のネタバレ抑制 + 結果画面飛ばしバグ修正
