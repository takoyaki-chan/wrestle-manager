# Stage A P3a 設計 — 断片連結テンプレ化+gameLog構造化 v0.1

- 作成: 2026-09-01(Fable)。P2監査(docs/i18n-stage-a-p2-audit-v0.1.md)の調査3・調査1-5/2-9への構造対処設計
- バッチ1(finType/総括見出し)は別途実装中。本書はバッチ2以降の正

## 共通原則

- **JA出力は1バイトも変えない**(ja-goldenが番人・--update禁止)。P3aは「同じ文章を、翻訳可能な構造から生成する」工事
- **完全文テンプレート化の方法**: 条件分岐で断片を連結している箇所は、**分岐の組み合わせごとに完全な一文のテンプレート**へ展開する(例: `{turns}ターンに及ぶ接戦の末、{phase}に{winner}が{finish}で{loser}を下した。`)。連結結果と1バイト一致するように各変種を列挙する
  - 変種数の上限目安は12。超える場合は既存の「。」境界でのみ文を分割してよい(文中での分割は不可=JA出力が変わる)
  - テンプレート置き場は data.js(NEWS_HEADLINE_TEMPLATES と同じ流儀)。プレースホルダは `{name}` 形式(WM_I18N.t仕様)
- **Engineは WM_I18N を呼ばない**。テンプレ選択と整形はEngine内でよい(データ駆動になっていれば、ENはStage Bでテーブルに英語列を足して表示時に引ける)

## バッチ2: 頂上戦記事+新聞断片連結のテンプレ化(監査3-1〜3-5)

対象と順序:
1. `_buildPpvSummitStory` matchPart(management.js:30778-30790)— 4段連結→変種列挙の完全文テンプレ表
2. `_buildPpvSummitStory` headline 6バリアント(30728-30744)— 既に変種型なのでテーブル移設のみ
3. PPVアンダーカード headline/body(31868-31891)— 所属括弧の条件挿入は「所属あり/なし」で別テンプレに分ける
4. 小規模連結4族(31776-31786/31826-31834/31955-31970/31988-32003)— 同法
5. `_generateNewspaperTexts` subheadline(app.js:9803-9811)— 3分岐×データ差し込み→3テンプレ

検証: ja-golden完全一致 / npm test / auto-sim 40(フックでも可) / ratchet非増加

## バッチ3: gameLog構造化+キーワードスニッフィング根絶(監査1-5/2-9)

**設計判断**:
- **D-G1**: 新形式エントリ `{ type: '<英数キー>', data: {...}, s: <season>, w: <week> }`。表示文は data.js の **GAMELOG_TEMPLATES**(type→完全文テンプレ。絵文字込みで現行文と1バイト一致)から表示時整形。`G.transferLog`(app.js:5571)が先例
- **D-G2**: **既存セーブの文字列エントリはそのまま許容**(計画決定#4「過去生成分は生成時言語のまま」)。表示側は「stringならそのまま表示 / objectならテンプレ整形」の二刀流
- **D-G3**: ログのUI分類フィルタ(ui-render.js:1014,4330等の `.includes()`)は新形式では `type` の族で判定。**旧文字列エントリに限りキーワード判定を残す**(レガシーはJA固定なので英語化しても壊れない)
- **D-G4**: 財務ラベル(ui-render.js:1816,1819)・タグ戦実況のCSSクラス判定(tag-battle-main.js:640-648)も同法: 生成元に category/type フィールドを足し、表示文の部分一致判定を消す
- **D-G5**: 約50箇所のpushは**機械的に一括変換**(1箇所=1 type)。typeキーは英数スネーク(例: `contract_signed`, `save_loaded`)。**Gに保存するのは type+data のみ**
- 派閥ロール・診断ラベル逆引き(監査2-8の6箇所)も本バッチで: 上流の生成箇所に中立キーを併記し、逆引き比較を消す

検証: ja-golden完全一致(gameLogは焼き込み→表示時整形に変わるが、**表示される文字列**は不変。goldenがdebugLog/gameLog文字列を直接スナップショットしている場合はスナップ対象を「表示時整形後の文字列」へ合わせる修正を同時に行い、その正当性を報告で説明すること)/ 既存セーブ読込の後方互換テスト(旧文字列エントリ混在セーブで例外なし)/ npm test / auto-sim 40 / ratchet非増加

## バッチ4(P3a残り): UIクローム一括 t() 通し

- ui-common/ui-render/app/index.html のボタン・ラベル・システム文(約4,700本)を機械的に `WM_I18N.t()` 経由へ
- 対象はプレイヤーに見える固定文字列のみ。ロジックキー(監査2の族)は辞書ヘルパー側で対応済みのものを除き触らない
- 大物量なのでファイル単位で小分けコミット。各ファイル完了ごとに ja-golden+walkthrough

## バッチ4の積み残し台帳(発見次第追記)

- `_dfcChronicle`(ui-render.js・黒田ナレーション生成)— 断片連結でproseを組む構造。バッチ2同様の完全文テンプレ化が必要(4a-3で発見・保留)
- `_chronicleCompetitiveValueHtml`(ui-render.js)— 「N度防衛」「N勝M敗」のJA構造文字列を正規表現で再解析する逆方向パターン。構造見直しが必要(4a-3で発見・保留)
- 数値直付けの単位語(`${n}回`「N勝M敗」等)— 英語の複数形問題と一体なので、Stage Bで単位テンプレ(`{n} wins`)として設計する。P3aでは非ラップで統一
- 「療養/休暇」文字列のCSSクラス兼用(ui-render.js・4a-1で発見・保留)— 表示とクラス名の分離が必要
- 表示兼ロジックキー2件(ui-common.js・4b-3で発見・保留)— `_agwRoleLabel`のロール名がroleRankのオブジェクトキー兼用/`_agwTeamViewState`の`label`が`=== '対戦中'`比較でCSSクラス切替に使用。中立キー分離が必要
- **断片連結の取りこぼし13群22キー(ui-common.js・P3b翻訳バッチ1で発見)** — 助詞・文片が個別にt()包みされた箇所(「が」「と」「から」「します。」等。ui-common.js 1505/4011/5191/7840/9964/10502/10571-73/10670-71/10986/11711/11768/11826/11898)。英語では語順が成立しないため翻訳不能。**呼び出し元を完全文テンプレート+プレースホルダに直す**(worklogのP3bバッチ1エントリに全リスト)。バッチ2で追加3群(「ロッカールームに満ちていた争いの空気は、」「不満を抱えている」「二人の対立は、」=既知群の前半部)+**「位」の序数問題**(ui-render.js:598・数字がキー外のため英語の序数接尾辞th/stが作れない。t('{n}位')→'#{n}'型へ呼び出し元修正が必要)+抽出器のhasProperNoun判定が字間空き表記(「天 頂 戦」)を素通しする穴。バッチ3で追加: 断片の頭・marker6キー(「対立が始まって以来、両派閥のリーダー」「新契約を結ぶと、」「明らかに和らいで」「水面下でくすぶっていた火種は、」「無益」「矛を収めること」+「抗争」が断片文中に人質)+**「案」の序数問題**(`${t('案')} ${kanji[i]}`で漢数字がキー外。t('案 {n}')型へ原文修正が必要=「位」と同型)
- `_agwMvpLine`のcontext分岐でMVPが3勝以上だと`gauntlet`が`champion`/`defiant`を飲み込み、秋MVPセリフ315行中210行が2勝以下のMVPにしか出ない(P5-2jで発見・仕様意図の確認待ち)
- CHALLENGE_LINESのJA重複11件(お嬢様と標準が「勝ちました。……ありがとうございました。」を共有=お嬢様の声が潰れる等・P5-2j)+GL-02-hostileの属性偏り(standard 17本/他6属性計8本)。JA側の書き分け候補
- renderJuniorTournamentMatchResult(ui-common.js:18305)の旧描画パス約90行が到達不能(P5-2iで発見・冒頭ブロックが必ずreturn)。掃除候補
- 「champion」timingをジュニア以外3大会(春タッグ18999/秋4団体MVP19441/天頂戦20472)が流用(P5-2iで発見)。JA増補時に「ジュニア色を足すと他大会で浮く」制約が非明文。仕様メモ化候補
- JA原文の重複12件(P5-2i・「……次」がstandard.quiet/cool.bold/composed.quietに等)— 属性違いで同一原文。書き分け漏れ疑い(P5-2f/2hと同族・裁定待ち)
- **M-24(ロッカールーム崩壊)のセリフが1行も無い(P5-2hで発見)** — relationships.js:1164でモーダルは積まれるのに空吹き出し「…」。セリフ執筆(JA+EN)の起票が要る=Keisuke裁定待ち
- ✅根治: 抽出器detectCellFromPathがdocId名の語尾(_seal_quiet等)を性格キーとして誤検出(P5-2hで発見・8行は手修正済み)。**2026-09-04、軸判定を「兄弟キー集合が丸ごとarchetype/personality語彙に一致するノードのみを軸とみなす」方式(`classifyAxisKey`+`resolveCell`)へ再設計して根治**。実データ全288トップレベルテーブルの全ノードを機械分類し、修正が影響する差分が台帳16,544キー中まさにP5-2hの手修正8行のみ(他の意図しない変化ゼロ)であることを確認済み(詳細はworklog)
- CAREの同一原文4件がセル違い共有(P5-2f引退2組と同族・JA書き分け裁定待ち)
- **`SNAPSHOT_TEXTS.breakthrough.scene` 1行が死蔵(P7-2で発見・P6-17で記録)** — `Engine.snapshot._collectCandidates` がブレイクスルーを常に `type:'embedded'` で積むため `voice` しか読まれず、`scene` 行に到達する経路が無い。訳出は済んでいる。**復活(=sceneを読む経路を作る)or 削除の裁定待ち**(既存の死プール群と同じ扱い)
- **✅解決(P7-28)** — **`RIVAL_ORGS.desc` 3文が死コード(P7-1で発見・P6-17で記録)** — ランキング画面の団体一言紹介文。変数 `deck` へ代入されるだけで一度も描画されない(P6-13の`_buildAceCopy`/`_buildLeadSentences`刷新で旧経路が置き換えられ、変数だけが取り残されたと推定)。→ `deck` 変数(ui-render.js ランキング画面)を削除。`RIVAL_ORGS.desc` 自体は新聞の団体比較号(P7-11でテンプレ化済み)で現役利用中のためデータ側は不触。JA出力不変
- **`ns.log`(ドラフト交渉ログ)がどこにも描画されない(P7-28で発見)** — `Engine.draftNegotiation.stepRound`(draft-negotiation.js:468/492)が `R{n}: プレイヤー降り ({bid}万)` 等をpushし、ui-common.js:6739の`draftPlayerAction`(降りるアクション)も直接 `ns.log.push(...)` する。しかし `G._draftNegotiation.negState.log` を読む箇所が src/test に1つも無い(交渉画面 `_renderDraftNegotiation` は `.round`/`.currentBid`/`.winner` 等のみ参照)。台帳化・翻訳とも未実施。**削除はしない**(消費点ゼロと確認済みだが、将来ログパネルとして描画する可能性もあるため)。**描画するなら要配線 の裁定待ち**
- **選択イベントS2/S6/E4型が生成経路なしで死蔵(P5-2gで発見・Keisuke裁定待ち)** — generateChoiceEventが3型を返さず、セリフ104行(訳済み)とUI側のtypeLabelが未使用。復活or削除の裁定が要る
- JA原文の「状況に噛み合わない汎用フィラー」30本(P5-2gで発見) — seductive×emotional「……っ……」同型22本(うち8本は社長の問いへの返事の形)+polite×shy同型8本。日本語側の書き直し候補
- app.js:13128の直書きフォールバック'…精一杯やります'が台帳外(現状不発・発火すればENでJAのまま)
- RETIREMENT_LINESに同一JA文がセル違いで2組重複(P5-2fで発見・日本語側の書き分け漏れ疑い): 「お泣きにならないで。みっともなくてよ」=A3_heelお嬢様normal/bold両方、「体がね…もう言うことを聞かないの…」=B2_primeお嬢様/蠱惑normal両方。ENは1本に統一済み。日本語の書き分けはKeisuke裁定待ち
- GLIMPSE_A_LINESの初期const宣言(~880行・data.js:25908)は直後の再代入で全上書きされる死コード(P5-2eで発見)。掃除候補(JA出力不変・goldenが番人)
- ~~VOLUNTARY_STAY 34行がENへ届かない~~ → **✅解決(70e6edd)**: pushをGAMELOG_TEMPLATES.voluntary_stay+advanceWeekのdict糸通し経由で組む形へ(ja不変・golden一致)。あわせてgameLogEntryTextをWM_I18N.t経由にし、EN時のログ内選手名ローマ字化が無配線で効くようになった
- 死コード2ブロック(ui-common.js・4b-3で発見)— ~~`_buildB3Step3b`の遮蔽された初回宣言~~(**✅P6-6で削除・2026-09-04**。JS仕様上どのみち後勝ちで常に生存側が実行されていたため挙動不変)/Junior・Tenchosen結果画面の無条件return後の到達不能尾部(未着手のまま)。P3a外で掃除候補
- **`COMMON7_LINES.resultLeader`(data.js)3行が消費点ゼロで死蔵(P5-2mで発見・P6-6で確認)** — `getCommon7Line('resultLeader', ...)`を呼ぶ箇所がsrc全体に存在しない。`showFactionCommon7Modal`は`coachReport`/`leaderAQuote`/`leaderBQuote`の3カテゴリしか引かず、結果画面は`applyCommon7Choice`(factions.js)が組む完全文`resultText`を使う別経路。訳出済み(EN)だが表示に届かない。合同企画の結果でリーダーが一言添わる画面を新設するか、このままデータとして残すかはKeisuke裁定待ち(新規UI配線は本バッチのスコープ外のため見送り)
- `SURVIVAL_MILESTONES`/`SURVIVAL_PHASES`(app.js・4c-1で発見)— モジュールロード時定数。`label`/`desc`はui-render.js:1204で`${m.label}`と非ラップ参照されており表示に乗る(`SURVIVAL_PHASES.label`は逆にどこからも参照されない死フィールド、これは対象外のまま)。他バッチの`FLAG_MODAL_META`/`_F01_ARCHETYPE_META`と同型の「宣言据え置き・参照側でt()評価」対応が必要だが参照側がui-render.js(既完走ファイル)のため本バッチ単独では直せず保留
- `Survival.updateSurvival`内`events.push('🎊 経営安定化達成！...')`(app.js・4c-1で発見)— 戻り値`events`を呼び出し側`checkSurvivalUpdate`が一切参照しない死コード。当該メッセージは別途エスケープ済みunicodeの`showEventPopup`側(同機能)でt()化済み。コード自体の削除はP3a外
- `applyDepartureTrustImpact`の`reason`引数(management.js全11箇所+app.js1箇所)— 自由記述のJA文字列(`'引退'`/`'AI怪我引退'`/`'突然退団'`等)がEngine側関数へ渡る。表示/内部判定どちらの用途か不明で、Engine本体(management.js)が対象外の本バッチでは1箇所だけ直すと不整合になるため全数非ラップ据え置き。Engine側のロジックキー整理が必要
- `_NEWSPAPER_HEADLINES`/`_NEWSPAPER_ARTICLES`(app.js・4c-1で発見)— 見出し/本文の完全文プールで`_generateNewspaperTexts`が抽選消費。バッチ2の`NEWSPAPER_SUB_TEMPLATES`と同種だが本体2プールは未着手のまま「対象外」の年代記・MVPレース文プールと同法(良い構造・Stage B翻訳対象)で据え置き
- ui-common.js:10433 `_mdlAReporterStrip`呼び出しの`opts.reporterText || '派閥について、ひとつご報告があります'`フォールバック — 同関数の他フォールバック(`payload.factionName || WM_I18N.t('派閥')`等)はt()化済みだがこの1箇所だけ4bで素通りしていた模様(4c-1でapp.js側の`reporterText`呼び出し元は全てt()化済みと確認済み)。ui-common.js側の1行修正はP3a外(ファイル対象外)
- `Engine.factions.getHostilityLabel`(factions.js・4dで発見・保留)— 表示兼ロジックキー。ui-common.jsの`hostilityBands`配列(冷え込み/小競り合い/抗争/泥沼/血みどろ)がこの戻り値を`indexOf`で位置判定に使っており、`getMomentumLabel`/`getSolidarityLabel`のような`getXKey`相当の中立キーが未整備。t()化するとen/pseudoで`indexOf`が不一致になるため非ラップ据え置き。中立キー分離が必要
- index.html ヘルプ画面(§遊び方ガイド)の長文パラグラフ約100件(4dで確認)— ほぼ全て`<strong>`タグで強調語を挟む混在構造。data-i18n はテキスト全体置換のため、`<strong>`だけ個別にt()化すると地の文とのバイリンガル混在になり逆に質が落ちる。バッチ2(頂上戦記事)と同様、分岐の組み合わせごとに完全な一文へ展開する構造対処(または`<strong>`をdata-i18n-html方式の別機構で扱う)が必要。純テキストの`<p>`(約38件)のみ4dでdata-i18n化済み
- `<title>WRESTLE MANAGER — 女子プロレス団体経営シミュレーション</title>`(4dで確認)— ブランド名(英語, 固有名詞)とJA副題の複合文字列。ブラウザタブ用でアプリ内UIクロームの範囲外と判断し非対象のまま据え置き

## 統合タスク: P3b構造スキップ36キーの原文再テンプレ化(4バッチ分の集約・✅完了 P3b-5 2026-09-02)

P3b翻訳バッチ1〜4が「原文側の構造問題で英訳不能」として登録したスキップ計36キーを、**1本のタスク**で呼び出し元修正する:
- 断片連結 約17群(助詞・文頭・文中markerの分割t()。ui-common.js 1505/4011/5191/7840/9964/10502/10571-73/10670-71/10986/11711/11768/11826/11898ほか)→ 完全文テンプレート+プレースホルダへ
- 序数・単位型2件: 「位」(ui-render.js:598→`#{n}`型)/「案」(`t('案 {n}')`型へ)
- 「🌸 総合ベストタッグ（第{n}回」(ui-common.js:4011)→ `t('🌸 総合ベストタッグ（第{n}回・{partner}と）')`
- 修正後に台帳再抽出(i18n-extract-ui)→旧キー掃除→新キー英訳→build-dict
- 検証: ja-golden完全一致(表示は不変)+build-dict green+walkthrough

**✅完了(P3b-5、詳細はworklogのP3b-5エントリ)**: 35キー(断片30個を16個の完全文テンプレへ統合+5個は入れ子t()のまま解放)+「天頂戦」を除くhasProperNoun=false未訳はゼロに。全て新規/追加で英訳済み。ja-golden(該当関数は対象外ファイルのため元々ノーカバレッジ)の代わりに、旧コード/新コードの出力文字列を全16箇所サンプル値で突き合わせるスクリプトを書いて1バイト一致を機械確認した。

## 対象外(P3aでやらない)

- 年代記・MVPレース文プール・compose*系(良い構造。Stage B翻訳対象)
- 休眠コンテンツ修復(年代記の性格フレーバー6種 management.js:6564)— **JA出力が変わる**(新しい文が出るようになる)ため P3a と分離し、単独の fix として golden 再採取込みで別途行う
- セリフ層・固有名詞辞書(Stage B)
