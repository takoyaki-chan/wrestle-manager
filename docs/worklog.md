...
検証: DOM検査で現行再現(OVR92=#f0d078+glow/SP107=色のみ)vs逆輸入(OVR92=純金太字glowなし/SP107=輝き)・枠越えPW112=88.4%+glow・ゴースト/マーカー・レーダー軸色mono→系統色・エラー0。残: Keisukeレビュー。承認後は選手ファイルと合わせて画面仕様書+Codex指示書(逆輸入は既存画面差し替えのため別タスク分割を想定)。
