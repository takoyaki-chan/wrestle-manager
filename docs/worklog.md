...
**申し送り3件へのFable裁定**: ①G1の±15%閾値は統計的に不成立(1.3σ)→χ²判定を正とする(承認) ②G4でsameOrgActiveRoster(孤立判定の母数)は不変条件優先で未変更→**採用**。休暇を母数から外すかはP2役割再定義で扱う ③G8のpair系エラー3件は汎用防壁で対応→**採用**(到達は異常系のみ、専用文言不要)。UI走破テストはマージ後にFableが実行。
