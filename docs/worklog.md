...
- **発見(スコープ外・チップ起票済み)**: 実セーブ棚 mobile_S22W47 の1季走破が年間表彰式 #aw-btn-next のクリック不能(D2)で失敗。**巡回OFFでも再現するため今回の拡張とは無関係の既存問題**(faa8071時点で再現、失敗stepが27/21/18とばらつく壁時計依存レース。ボタンは可視なのにPlaywrightのactionability待ちが通らない)
