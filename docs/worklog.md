...
## 🌐 P4-1黒田英文体プロトタイプ+P5先行アンカー102本英訳 完了（2026-09-02・Opus主筆/Fableレビュー）

- **黒田英文体** [en-kuroda-style-draft-v0.1.md](docs/en-kuroda-style-draft-v0.1.md): 三層主語(本紙=this paper/筆者=this writer/漏れる心情=I)/「持ち上げて落とす」は文を切る/慨嘆4道具/紙面の三声(幸子・貫一郎・無署名デスク)を潰さない/見出し文法(冠詞省略・現在形・sentence case)/プレースホルダ安全則「値に文法を決めさせない」/禁止語grep+maxim検査。対訳見本20本つき。**最大の発見=成形済み値約20個**({milestone}=「通算100勝」等)と関数内プールの抽出穴→P4設計に追記、P4-2で棚卸し中
- **アンカー34セル×3本=102行英訳** [en-anchor-samples-draft-v0.1.md](docs/en-anchor-samples-draft-v0.1.md): A/B判定済み31行は無変更転記+収束20行の採用形選定+新規46行。寡黙4セルを「短さの意味」(断片/完結短文/判決/確認)で書き分け、強気3セルを「誇りの矛先」で分離。機械検査(110字以内/ALL CAPS 0/卑語規律/お嬢様無短縮形)済み。**ネイティブ検品(Patreonフォロワー)への提出パッケージ=本書+黒田見本**
- 固有名詞裁定4問確定(0816c0b)を受け、P3b-6(固有名詞90キー英訳)とP4-2(テンプレ台帳+配線)がworktreeで進行中
...
規範: `docs/i18n-stage-b-p3b-design-v0.1.md` D-B3(ボタン・見出し=Title Case/末尾ピリオドなし、メッセージ=sentence case、絵文字は原文位置)+用語集シード20語 / `docs/en-tone-bible-draft-v0.1.md` §0最重要則2(忠実さより英語としての自然さ)・§1鉄則(米綴り・♪♡存置・卑語禁止)・§4-6翻訳調検査リスト。UIクロームなので§2の属性ボイスは不使用。

### 成果
- **充填 778行 / スキップ 22行**(台帳総キー3,123・訳文あり778・未訳2,345はfail-open)
- 固有名詞入り89行は**手つかず**(D-B5: 固有名詞辞書がKeisuke承認待ちのため)

### スキップ22行 — すべて「断片連結」(原文側の構造問題・具体的リストとして起票)
`specs/i18n-runtime-spec-v1.0.md` §2-3 が禁じている**文の断片をt()で切って連結している箇所**が、factions系の社長室イベント文を中心に13グループ残っている。日本語は助詞が後置なので断片のまま並ぶが、英語は語順が入れ替わるため**スロット単位の置換では文にならない**。訳を当てても壊れるだけなので訳さず、P3a積み残し台帳へ回す前提で列挙する(いずれも `src/ui-common.js`):

| 箇所 | スキップしたキー | 理由 |
|---|---|---|
| 1505 ロスター超過 | 「します。」 | 述語だけの断片。英語では文頭の助動詞に化ける |
| 4011/10670/11826 | 「と」 | count=3で意味が割れる(and / with)。1語キーに1訳を焼くと他の2箇所が壊れる |
| 5191 遠征バー | 「が」「のリングへ乗り込む」 | 1文字助詞キー。" invades " を焼くと将来のt('が')が全部誤訳になる |
| 7840 レンタル供給元 | 「から」 | 英語は前置詞が先行するため位置が合わない |
| 9964 派閥誕生 | 「を中心として、」 | 目的語の後置助詞 |
| 10502 引き抜き察知 | 「は」「と既に話をつけているようです。」 | 主題助詞+述語の分割 |
| 10571-10573 派閥内不満 | 「はリーダー」「の方針に」「ようだ。」 | markerの「不満を抱えている」が英語では対象の前に来る |
| 10670-10671 和解の兆し | 「の間にあった棘は、」「ここ数週間で」「いる。」 | 副詞句とmarkerの前後が反転する |
| 10986 敵対段階 | 「まで来ている。」 | 同上 |
| 11711 点火 | 「にまで燃え広がった。」 | 同上 |
| 11768 停戦 | 「ただ、〜抗争が続くことの」「を知り、」「を選んだ。」 | em強調2つを挟む3分割 |
| 11826 決着 | 「という、はっきりとした形に決着した。」 | 同上 |
| 11898 終わらない抗争 | 「は決着がつかないまま、」「を食いつぶし続けている。」 | 同上 |

一方、**名前の後ろに付く後置句**は英語でも同じ位置に置けるので訳した(「への引き抜き交渉を開始します。」→ " will be approached about a transfer." 等・計8キー)。

### 新規訳語決定リスト(用語集シード20語にない頻出語。台帳内で一貫させた)
| JA | EN | 補足 |
|---|---|---|
| 因縁 / 宿敵 / ライバル | grudge / nemesis / rival | 3語を訳し分ける。rivalryは派閥devラベルのみ |
| 絆 / 対立度 / 勢い / 結束 / 求心力 | bond / hostility / momentum / cohesion / pull | factions系ラベルの統一 |
| 火種 / 抗争 / 決着 | spark / feud / settled | 「火種は熟している」= the spark is ready to catch |
| 知名度 | renown | 団体・派閥どちらにも使える語を選定 |
| 集客力 | draw | 「🎤集客力順」= By Draw |
| エース力 / 層の厚み / タイトル力 / コア戦力 / 看板スター | Ace Power / Depth / Title Power / Core strength / Marquee star | 団体力4軸 |
| 決裁 / 決裁枠 / 書類 | approve / approvals / document | 用語集の⚡approvalsを動詞・名詞へ展開 |
| 通常興行 / 特別興行 | regular show / special show | 興行=showの派生 |
| 遠征 | away | 遠征対抗戦 = away interpromotional match |
| レンタル | loan | rentalにしない(プロレス/スポーツの慣行語) |
| 殿堂入り / ゴールド殿堂 / レジェンド | Hall of Fame / Gold Hall of Fame / Legend | 3段の格 |
| 新人王 / ベストマッチ / メディア功労賞 | Rookie of the Year / Best Match / Media Merit Award | 表彰名 |
| JT | JT (= Junior Tournament) | 略記は維持。初出は "U-20 Junior Tournament" |
| S級/A級/B級/C級 | Class S / A / B / C | コーチ等級 |
| 先鋒・中堅・大将 | Lead-off · Middle · Captain | 3人制の出場順 |
| 序列(戦) / 格付け争い | rank / ranking match / fight over rank | 派閥内の順位争い |
| 体調 / 直訴 / 起案 / 挑発 / 遺恨 | condition / direct appeal / proposal / taunt / bad blood | |
| おまかせ / おまかせ編成 | Auto / Auto Booking | |
| 「〇〇派」 | the 〇〇 Group | 派閥名 |
| 追い込み / ⚡追込練習 | hard drilling / ⚡ Hard Training | |
| 人気(狭いチップ) | Pop | 散文では popularity。OVR/Pop/Age の並びのみ短縮 |

### 要裁定1件: 通貨単位「万」
英語には「万」がないため **`{v}万` → `¥{v}0k`** とした(1万=¥10k、3,000万=¥30,000k)。数値を再フォーマットせず末尾に "0k" を足すだけなので**桁は常に正確**という理由での採用だが、3,000万級が "¥30,000k" と冗長になる。正規の数値フォーマッタを入れて "¥30M" 表記にするかはP6(表示系)の宿題としてFable/Keisuke裁定を仰ぎたい。現状はリテラル(目標3,000万→¥30,000k)も同じ規則で揃えてある。

### 表記の整理(英文に全角約物を持ち込まない)
`・`→ 箇条書きは `•` / 区切りは `·`、`｜`→`|`、`／`→`/`、`＋`→`+`、全角空白→半角空白 に正規化。**絵文字・記号(⚔⚡★⛔✍等)は原文の位置のまま維持**。
全角スペースで字間を空けた儀式ボタン(「— 決 裁 す る —」等16本)は、英語では**字間空けを再現しない**(— Approve —)。英語のletter-spacingはCSSの仕事で、リテラル空白で組むと折り返しが壊れ幅も倍増するため。

### 検査系の修正2件(いずれもtest/配下・srcは台帳生成物のlang-en.jsのみ)
1. **`test/i18n-build-dict.js` のJA残り検出が絵文字を誤検出していた** — 既存のJA判定レンジ `[぀-ヿ㐀-鿿豈-﫿ｦ-ﾟ一-鿿]` は、`豈` が互換漢字 U+F900 ではなく通常漢字 **U+8C48** に潰れており、実効レンジが U+8C48〜U+FAFF まで広がって**サロゲート(U+D800〜U+DFFF)を巻き込む**。結果 🏋 🔥 🤖 🏛 のようなBMP外絵文字を含む訳文が全部「日本語が残っている」で弾かれた。D-B3が絵文字の原文位置維持を要求している以上この誤検出は踏めないので、**コードポイント明示** `[぀-ヿ㐀-䶿一-鿿豈-﫿ｦ-ﾟ]` に置換(理由コメント付き)。`・`(U+30FB)は引き続き検出されるので上記の約物正規化で対応した
2. **`test/i18n-scan.js` の走査対象から `src/lang-en.js` を除外**(理由コメント付き) — lang-en.jsは台帳から生成されるEN辞書で、中の日本語は**翻訳キー(原文)**であって移行すべき直書きリテラルではない。除外しないと翻訳を進めるたびにラチェットが増加で落ちる(実測 0→772)。P3b-1のworklogが予告していた懸念の実地対応。**基準の`--update`ではごまかしていない**(除外後の実測で increase 0 を確認)

### 検証(全項目実施)
- `node test/i18n-build-dict.js` — 機械検査green(プレースホルダ完全性/重複キー/日本語残り)。`src/lang-en.js` 生成: 総キー3,123・訳文あり778・未訳2,345
- `node --check src/lang-en.js` — 成功
- `node test/ja-golden.js` — **完全一致**(hash `6b3d05c8...` 不変)
- `node test/i18n-ratchet.js` — **増加なし**(files=31 totalJaStrings=28075)
- `npm test` — **260/260 PASS**
- `npm run test:ui:walkthrough` — **PASS**(下記)
- **ENモード実動作の抜き取り確認**: vm上で `src/i18n.js` + `src/lang-en.js` を実ロードし、`setLang('en')` 後の `t()` 出力を代表20キー+params置換+未訳fail-open+ja復帰の**23件検証しall OK**(使い捨てスクリプト)

### 残課題
- 次バッチは hasProperNoun=false の **801行目以降(残2,256行)**
- 固有名詞入り89行は名詞辞書のKeisuke承認待ち(`docs/en-proper-nouns-draft-v0.1.md`)
- 上記スキップ22行の断片連結は**原文側の再テンプレート化**が要る。P3a積み残し台帳に追記して別工程で処理する
- 通貨「万」の表記(¥{v}0k)は要裁定
...
### 対象と方針
- 年代記: セクション見出し(「旗揚げ世代」「この時代の主な出来事」等)・ステータスバッジ(「初代王者」「看板」「進行中」「確定」等)・空状態文言・ナビボタン(「前章なし」「次章なし」等)をラップ。**章タイトル(`<h2 class="chron-title">`)・プロローグの記者コメント本文・章末クロージングラインのフォールバック文は narrative/Stage B領分として明示的に対象外のまま維持**(current.title/current.subtitle等G由来変数は元々ラップ対象外)
- `_dfcChronicle`(黒田ナレーション生成関数、抗争/中立の分岐でprose文を組み立てる)は断片連結型のnarrative composeであり、Batch2で扱った「完全文テンプレ化」相当の再設計が必要なため本バッチでは対象外のまま(スキップ理由をコード上のコメントでなく本ログに記録)
- `_chronicleCompetitiveValueHtml`(正規表現でJA文字列"N度防衛"/"N勝M敗"を解析し直す関数)も構造的に文字列パースへ依存しており単純な t() ラップでは対応できないため対象外
- 派閥: 空状態(封印中/派閥ゼロ)・セクションラベル・カードの役職/勢い/結束ラベル・抗争タイムラインのイベントラベルをラップ
- 相関図: モバイル版のセクション見出し・関係ラベル・検索プレースホルダ、デスクトップ版のヘッダー(ネットワーク/フォーカス/勢力図トグル)・凡例・ズームコントロール・コンテキストメニュー・詳細パネル・比較ポップアップ・勢力図(power view)の団体ラベル/凡例キャプションをラップ
- EMOTION_TEXTS(感情セリフ辞書、archetype別)はセリフ層としてStage B対象のため不変

### スキップ判断のまとめ
1. 章タイトル・記者コメント・クロージングラインのフォールバック文 — narrative/Stage B領分(タスク指示の明示的除外)
2. `_dfcChronicle` — 断片連結prose、Batch2型の完全文テンプレ化が必要(構造変更なので本バッチのスコープ外)
3. `_chronicleCompetitiveValueHtml` — 正規表現でJA構造文字列を解析する関数、構造ごと見直しが必要
4. 数値に直接後置される単位語(「回」「勝」「敗」「戴冠」「期」「名」「度」等、`<strong>${n}</strong>回`のような直付けパターン) — 4a-2委譲済みのHoF詳細セクション(10257-10260行)で既に同型が非ラップのまま確定していた先例に合わせて統一
5. EMOTION_TEXTS — セリフ層(Stage B)
6. G由来の動的文字列(current.title/h.text/prologue.closing等) — そもそも文字列リテラルではないためラップ対象外(変数)

### 検証
- `node --check src/ui-render.js` — 成功
- `node test/ja-golden.js` — 完全一致(hash `6b3d05c8...` 不変、全編集を通して崩れなし)
- `node test/i18n-ratchet.js` — 直書きJA文字列の増加なし(files=31 totalJaStrings=28378)
- `npm test` — 260/260 PASS。**1本(mobile-layout-test.js)がソースパターン照合で割れて修正**: 相関図モバイル版のセクションラベルが絵文字+t()呼び出しに分割されたことで`'🔥 因縁・ライバル'`のような連続リテラル一致が崩れたため、`"🔥 ${WM_I18N.t('因縁・ライバル')}"`型の照合へ前任バッチ(f09-empty-slot-lock-test等)と同じ流儀で追従修正
- `npm run test:ui:walkthrough` — **PASS**(328操作・issues 0・季末S2W1まで到達・実機画面遷移も正常)

### 数値
- `WM_I18N.t()`呼び出し: ui-render.js全体で1,146→1,344(**+198箇所**、本バッチ分)
- コミット: 6本(意味単位で分割 — 年代記+派閥/相関図モバイル+デスクトップ骨格/相関図SVGラベル+勢力図/相関図単体団体ビュー+詳細パネル/比較ポップアップ/テスト追従修正)

### 残課題
- P3aバッチ4a(ui-render.js)は本バッチで完了。**残るバッチ4bはui-common.js/app.js/index.html**(設計doc: docs/i18n-stage-a-p3a-design-v0.1.md §バッチ4)
- 実機確認: データベースタブの「年代記」「派閥」「相関図」(PC/モバイル双方)を英語モードに切り替えず通常表示で一通り開き、レイアウト崩れ・文言欠落が無いか確認いただけると安心(JA出力は不変のはずだが、t()呼び出しの参照ミスが無いかの最終目視)
...
### P1: i18n基盤(worktree実装→Fableレビュー→mainマージ)
- 設計の正: [i18n-stage-a-p1-design-v0.1.md](docs/i18n-stage-a-p1-design-v0.1.md)(設計判断D1〜D10+不変条件I1〜I6)。実装はSonnetエージェント(worktree・本体ツリー非接触)、Fableがdiff全件レビュー後マージ
- **src/i18n.js新設**: `WM_I18N.t()`(キー=日本語原文。ja時は辞書非経由の素通し=I1・en時はfail-open+翻訳漏れログ・pseudo時は`⟦原文~~⟧`)+localStorage `wm_lang`(セーブGに入れない)。index.htmlの全スクリプト先頭で読み込み。**release/manifest.json追記済み**
- **JAゴールデンテスト** test/ja-golden.js: 固定シード20季のエンジン生成テキスト(新聞・ティッカー・決着文・引退文・debugLog等11,233行)をSHA256スナップショット。**マージ時にスランプ修正(下記)前の基準と不一致を正しく検知→検出器の生存確認になった**。基準は修正後mainで採り直し(hash 6b3d05c8…)
- **ラチェットガード** test/i18n-ratchet.js: i18n-scan流用のファイル毎生日本語本数基準(31ファイル・28,819本)。増加で失敗
- dev-toolsパネルにja/en/pseudo切替(プレイヤー動線には出さない)。パイロットとしてui-render.jsの画面ヘッダ8種をt()経由に
- 検証: npm test **259/259** / auto-sim 40季 ALL CLEAR / ja-golden一致 / ratchet OK。**日本語版の見た目・挙動は不変**
### P2: 監査(読み取り専用エージェント→Fable検算→spec化)
- 成果物: [i18n-stage-a-p2-audit-v0.1.md](docs/i18n-stage-a-p2-audit-v0.1.md) = **P3a抽出工事の作業台帳の正**
- 要点: 焼き込み5大分類(新聞2種・年代記・MVP・gameLog50箇所超。G.news/記録タブ/殿堂はクリーン)/ロジックキー実測107件8族(最危険=`.includes('興行')`型キーワードスニッフィング約25件。翻訳で無音故障するため構造化フィールド追加が必要)/断片連結は7箇所に集中(筆頭=頂上決戦記事のSOV語順ロック)。**年代記・MVP文プールは構造工事せずStage B翻訳に回す**のがP3a 6〜10セッション死守の鍵
### 副産物: 既存バグ修正+休眠コンテンツ発見
- **fix(5450e3c)**: management.js:12630 三項演算子書き損じ(`: '中傷' ?`)で軽傷・練習負傷の復帰も「中傷復帰」扱いのスランプ判定が走っていた。一行修正・auto-sim 40季 ALL CLEAR。軽い怪我明けのスランプがやや減る方向
- **休眠コンテンツ**: 年代記の性格別フレーバー文6種がmanagement.js:6564で非実在の性格キー(aggressive等)分岐のため一度も表示されていない。P3aで現行キー(bold等)へ修復予定
- 残課題: P3a(抽出工事・監査の推奨順で着手)。実機確認事項は特になし(見た目不変のため)。specs反映はStage A完了時にまとめて

## 英語トーンバイブル較正完了 — 裁定12件反映+A/Bブラインド50本で量産体制を確定（2026-09-01・Fable）

朝の試作起草(下記エントリ)から同日中に、Keisukeレビュー・裁定・A/Bブラインドまで完走し、トーンバイブルを「較正済みv0.1」に到達させた。**Stage B(翻訳)着手判断の材料はこれで完備**。

- **裁定の反映(計12件)**: §6の6件(方言コスプレ禁止/卑語hell,damnのみ/♪♡存置/呼称Boss/米綴り/表記個体差)+大河内キャラ裁定(主張しない・courtesy without deference・敗北帯は締まる)+A/B判定から抽出した5件——①はしゃぎは反復・伸ばしでなく素直な一回 ②疑問文には疑問符(疑問形ピリオド不採用) ③「……」後の新文は大文字 ④試合後の評は過去形 ⑤**最重要則2「忠実さより英語としての自然さ」**(素直に訳しすぎて不自然になるなら意味を保って言い回し自由)
- **A/Bブラインド(Fable 5 vs Opus 5・計50本)**: 第1弾=未訳25セルのアンカー固有台詞30本([en-ab-blind-30](docs/en-ab-blind-30-v0.1.md)+差分10行判定シート[focus](docs/en-ab-blind-focus-v0.1.md))、第2弾=Keisuke指名のアンカー9名24本([anchors9](docs/en-ab-blind-anchors9-v0.1.md)・こちらは**見本英訳を伏せた別バイブル**をOpusに渡して公正担保)。ブロック毎にA/B入替・キーはscratchpadに封印→判定終了で開示。**結果: Opus 14勝/Fable 14勝の完全互角**。約1/3は同文収束=筆でなくバイブルが声を決めている
- **量産体制の結論(バイブル§5-1)**: 主筆=Opus 5(確立ワークフロー一致・コスト半分)/Fable=バイブル番+全行レビュー/第三層=**ネイティブスポットチェック**(KeisukeのPatreonフォロワーにキャラに詳しいアメリカ人がいることが判明・初回検品で翻訳調2件を実検出: 「今日の私」型・「〜も」too直訳型→§4-6検査リスト化)
- **見本の勝ち稿差し替え**: バイブル§2の対訳テーブルを全て判定の勝ち文面へ更新(高津2本→Opus稿、宇田川→"A hot girl never slacks off"、大河内3本→Opus稿+Y大文字、富岡#7に疑問符、阿武隈#21過去形など)
- 触ったファイル: docs/en-tone-bible-draft-v0.1.md / en-ab-blind-30 / en-ab-blind-focus / en-ab-blind-anchors9(全てdocsのみ・コード変更なし=auto-sim対象外・manifest対象外)
- **残課題**: Stage Bゴー判断はKeisuke(順序はStage A翻訳可能化工事が先)。ゴー時にバイブルをspecs昇格+INDEX追記→アンカー34セル×3本(§5-3)から量産開始

## 英語トーンバイブル試作 v0.1 起草 — Stage B着手判断の材料（2026-09-01・Fable）

英語対応計画(docs/i18n-english-plan-v0.1.md)の「Stage B(翻訳)は英語トーンバイブル試作後に別途判断」を受け、試作を起草。モデル選定議論(セリフ翻訳の筆はFable/Opusが最有力、量産前にA/Bブラインドで決着)から「その判断レベルの仕事=トーンバイブル設計」に着手したもの。

- **成果物**: [docs/en-tone-bible-draft-v0.1.md](docs/en-tone-bible-draft-v0.1.md)(新規)。規範元は specs/dialogue-tone-spec-v1.0.md+34セル口調シート+アンカー表
- **中心設計**: 日本語は文法(語尾・一人称・敬語)で声を作るが英語にはどれもない → **語彙(register)・構文の形・リズム**の3ダイヤルへ翻訳する対応表を背骨に据えた。最重要則は「**翻訳は温度を上げない**」(Keisuke採用実績のある抑制の効いた筆致を正とする)
- **内容**: 全属性共通の英語鉄則10条(JA spec §3の英語版+方言コスプレ禁止等の英語固有則)/7属性別ボイスレシピ+アンカー見本の対訳21本(高津・宇田川・富岡・堂前・本郷・川野辺・阿武隈・橘)/性格7種の英語変調表/英語固有の危険5項(温度引き上げ・翻訳調・声の均質化ほか)/検証計画(A/Bブラインド30本→アンカー100本→Excel英語列量産)
- **裁定待ち6件**(§6): お嬢様の方向(推奨=方言コスプレなし・無短縮形+格調語彙)/卑語上限(hell/damnをヤンキーのみ)/♪♡存置(推奨=残す)/社長の呼称("Boss"標準)/米綴り統一/表記個体差(ワタシ等)の扱い
- **検証**: コード変更なし(docsのみ)のためauto-sim対象外。manifest対象外(src配下変更なし)
- **残課題**: Keisukeレビュー+裁定6件 → 承認後にA/Bブラインド(Fable 5 vs Opus 5、30本)で量産の筆を決定 → specs昇格
...
**マージ時の一本化(Fable)**: MQ移行の穴は本日4セッションが独立発見(522adda / lucid-haslett / stoic-morse / 本worktree)。mainは「v1一般形拡張(522adda)+v2追いマーカー(lucid)」の二段構えを採用し、本worktreeのv2単一ブロック実装は落として一本化した。mq-text-migration-v2-test(実セーブ由来の焼き込み文字列で移行を検査)はmain版v2に対して採用。
...
処置後の再走破: **6本中5本✓**。残る1本(v1.0x世代S2W23、S2W48 ppvTVで進行不能・popup queue=1 active)は**実プレイヤー影響が疑われる本物のフリーズ**として、worktree隔離のエージェント(task-103)で根治した(次エントリ)。
...
### マージ後の追加修正(2026-08-31・Fable): 移行の取りこぼし3種
本エントリ時点の「移行が実セーブ6本で機能」は**新聞・MVPレース画面を開かない走破**での判定だった。並行マージされたナビ巡回(9画面D3走査)×修復済みの棚が組み合わさった初回統合走破で、S73超長期セーブの新聞から取りこぼしを検出(検査網の掛け算が機能した実例)。①記事本文の「、MQ 84を記録」「MQ 96——」型(読点直後/文頭)がどの移行regexにも不一致 → 一般形 `\bMQ (?=\d)`/`\bMQ(?=\d)` を最後尾に追加(識別子は語境界が立たず無傷) ②`currentNewspaper` ③`mvpRace`(narrative)がキー一覧から漏れ → 追加。オフライン監査(移行再現スクリプト)で実セーブ6本の残留ゼロを確認後、統合棚走破で最終確認。
...
### 発見・修正した検出器の欠陥(すべて即日修正)

1. **auto-simがISSUES FOUNDでもexit 0**(終了コードを見る全消費者に偽緑) → `process.exit(allClear?0:1)`
2. **run-allのviolations正規表現が実出力に構造的に不一致の死文**(合否がALL CLEAR文字列の一本足) → 実形式へ+exitコード一次判定に二重化
3. **資金NaNの無言洗浄**: sanitizeFloatsの`||0`が検査前に0円化(変異nan_fundsで「入口→出口の間の週次再計算でも洗浄される」ことまで実証) → **tickWeek入口検査**+sanitize側の記録義務化。全財産が黙って消える級の破損が必ず痕跡を残すように
4. **観客数検査が幻のフィールド参照で恒偽**(`_lastAttendance`は誰も書かない。実在は`lastShowAttendance`) → 配線し直して蘇生
5. **decisionPoints上限検査が検査対象自身のmaxを借用**(dp=15/max=15が素通り) → 設計値6の独立コピーで縛る
6. **台帳L1が検査対象の定数を借用して変異で盲目化**(前エントリ) → 独立コピー化(LEDGER_L1_CAP)
7. **ロスター重複検査が未実装**(Setをaddするだけでhas()が無い) → 実装(WM_CHAOS=dupe_rosterで発見)
8. **D1が`[WM]`自己修復ログを素通し**(壊れて黙って立て直した走破がPASSになる) → flight-recorderと同基準へ
- 付随修正: D3のNaN誤爆時限爆弾(case-insensitive部分一致→厳密+単語境界。英語化前に解除)/計測専用スクリプト(assert 0件・毎回40秒)をnpm testから除外(`measure:stats`へ)/salary-refix-testに手書きリテラルの給与式ピン追加(結合物差し対策)/manifest未記載警告が**実装以来一度も動いていなかった**(PS5.1のGet-ChildItem -Include罠)→蘇生+ERROR化(前エントリと統合)

### 常設化した自己点検(いつでも1コマンドで検出器の生存を再証明)

- **`npm run test:detectors`**(約5分): 変異2種(WM_LEDGER_MUTATION)+カオス4種(WM_CHAOS=nan_stat/dupe_roster/throw_tick/nan_funds)は**鳴くのが合格**、健全20季+balance-baseline+走破self-testは**静かが合格**の9本マトリクス。初回実行が上記3(nan_funds未検出)を現行犯で捕まえた実績あり
- **`npm run test:save-regression`**: 実セーブ棚(test/ui-walkthrough/fixtures/legacy-saves/、Keisuke提供の実セーブ6本: 3月v1.0x世代〜8月v1.25世代・S73超長期・スマホ版含む)へsave-doctor診断+`--walkthrough`で実ブラウザ1季走破。「検査は毎回まっさらな新規ゲームしか通らない」死角(v1.32給与バグの住処)への恒久対応。初回診断の収穫: 実セーブに重複ID実在(モバイル15件/v1.20 1件)
- **MQトークンをD3に追加試行→実在の残存違反14箇所を全fixtureで検出**(週次ログ・DB対戦履歴列・因縁ベスト試合等の生MQ表記)。掃除は文言レビューが要るため専用チップ起票、完了後にトークンを戻す(detectors.jsコメントに明記)

### 監査で確定した既知の限界(正直な申告・修正せず記録)

- テスト257本の実質: 値の破損まで捕まえられる級は15〜25本、約6割はソース文字列存在検査(「書いてあるのに出ていない」型に構造的に盲目 — その型はUI走破・点火カタログの管轄)
- 走破ハーネス: ナビ配下の自由閲覧UI(社長室机・DB・新聞等)に構造的に到達不能(NAVIGATION_TEXTが絵文字不一致の死にガード)→チップ起票/「進んだが内容が間違っている」型・見た目の破綻(overflow切れ)・兄弟ボタンで隠れる死にボタンは対象外
- validateGameState: 修理層(sanitize系)が検査の前に走る「無言修理」構造が残存(影1〜4)。funds以外の修理記録義務化は将来課題/factionsブロックはauto-simで恒久未実行(通常simは派閥最小人数に届かない)
- balance-baseline(最強の独立検出器)がどの自動入口からも呼ばれていなかった→test:detectorsに組込済み。--saveの上書き安全化とコミットハッシュ記録は未対応
- 台帳: ゆっくりした漂流はL1の跳び検査を潜る/資金恒等式は未実装(チップ)

## v1.34: 給与爆上がり報告の根本対応+台帳検査層の新設（2026-08-31・Fable直実装）

アンケート最新回答(08/31・v1.32)「契約更改+25万のはずが47万→336万/交渉なしでも63万→251万、契約更改でフリーズ→勝手にオフ3/4へ」への対応。調査(エージェント委託・全実測)で真因確定: **計算バグではなく移行の穴** — 契約再固定はv1.14〜v1.29の間「交渉発生季には一度も実行されない」バグで死んでおり(選手シーズンの92.7%がgap≥1.3)、v1.30の復活により旧版セーブの数季分の査定ギャップが初回オフシーズンに無表示で一括精算されていた(式で報告値を正確に再現。健全セーブの1季追従は実測max×1.53)。

- **修正1(refixRoster・management.js)**: 上向き改定に季あたり上限 `SALARY_REFIX_CAP`(旧基本給×1.6+10万)。超過時はcontractOVR/contractPopを旧→現在値の線分上で部分更新(二分探索)し数季で段階追従。下向き(下り交渉カード管轄)は無制限のまま。健全セーブは上限に触れず挙動不変
- **修正2(offWeek3・可視化)**: 再固定はどの画面にも出ていなかった → 週次ログに「📋 契約査定の改定(週給合計±X万)」+20万以上動いた選手の内訳(最大8行)
- **修正3(契約チェーン看視・app.js)**: §5-D鉄則1欠落の対策 — 例外でセッションがactiveのまま死ぬと全入口が無言returnし恒久フリーズしていた。慎重2段構えの看視(DOM不在=常に生存扱い/.neg-btn不在が2回連続+1.5秒続いたときだけセッション破棄→毎手保存カーソルから再開。誤検知でも同一性検査で二重適用なし)。「勝手にオフ3/4へ」の機序は未確定(フライトレコーダー記録待ち)
- **台帳検査層の新設(test/auto-sim.js・Keisuke指弾への恒久対応)**: 「公称と実効の乖離」族(給与/招聘0pt/合宿2倍/MN無意味)は状態健全なので全検査層をすり抜けプレイヤーにしか見えなかった → 第4層として常設。L1=給与連続性(基本給が連続観測中に上限超で跳んだら違反)/L2=更改の約束(交渉1件で±60万超動いたら違反)。ALL CLEAR判定に組み込み
- **検出能力の両側証明**: `WM_LEDGER_GAPTEST=1`(旧版セーブ合成: S2W1に上位3名へギャップ注入)で、修正前エンジン(WM_SOURCE_REF=HEAD)=**違反4件ISSUES FOUND**(=報告バグを自力検出)/修正後=**違反0で段階追従ALL CLEAR**
- **検証**: salary系3テスト改定(上限内=従来一致/超過=上限+8季収束+単調性)+npm test **257/257** / auto-sim 40季42 なし・--care両ALL CLEAR(**台帳32,245件検査・違反0=偽陽性なし**) / 走破(実行中) / version-consistency 1.34 ok
- 教訓の恒久化: メモリ`feedback_promise_outcome_ledger`(数字を見せたら台帳で検算・恒等式なしの数字約束機能は完了扱いにしない)
- **変異自己点検の常設(同日追補・Keisuke「検出器が本当に機能しているのか怪しい」への回答)**: `WM_LEDGER_MUTATION=<refix_uncap|raise_double>` — わざと壊したエンジンで走らせ**ISSUES FOUNDが出るのが合格**という逆向きスイッチ。初回実行で**検出器自身の欠陥を1つ発見**: L1が検査対象と同じ定数(SALARY_REFIX_CAP)を参照しており、上限撤廃変異で検出器の物差しも一緒に壊れてALL CLEARが出た(監視役が被監視者の定規を借りる連動の罠)→ **L1に独立コピー(LEDGER_L1_CAP)を持たせて修正**。修正後: refix_uncap=L1違反4件✓ / raise_double=L2違反30件✓。L2も±60万の粗い網から**実合意額(resolveNegotiationのsalaryDelta)との±2万照合**へ強化(2倍付与級を確実に捕捉。旧網は典型昇給10〜25万の2倍=20〜50万を見逃す粗さだった)
- **既知の限界(正直に記録)**: L1は「跳び」の検査であり、毎週+0.5級のゆっくりした静かな漂流は潜り抜ける(季スケールのドリフト検査は将来課題)。funds恒等式は未実装(チップ起票済み)
- 残: 経過観察=08/22報告「オフ2段階目で停止」はv1.32修正で解消の可能性(次の回答を監視)

## リリース前フルテスト+バグ2件修正 — v1.33配布準備（2026-08-31・Fable）

Keisuke指示「徹底的にテストプレイ→修正→デプロイ→+0.01で梱包」。テストバッテリー: npm test 257/257×2 / test:quick 112/112 / auto-sim 100季42 ALL CLEAR(5300週・違反0・Game overs 0) / auto-sim 40季42 --care ALL CLEAR / UI走破(修正後PASS・316操作・Issues 0・S2到達) / レア画面点火 tenchosen+gameover 両PASS / bug:audit指摘0。**走破が実バグ2件を炙り出し、即日修正**:

- **修正1(management.js validateGameState)**: ロスターキャップ不変条件が挑戦試合・遠征・統一王座戦・乱入・奪還のゲスト(isCRGuest等6フラグ族)を頭数に数えていた — ゲストの居る興行週だけ偽陽性で鳴る。セーブ除外(app.js serialize)と同じフラグ族で正所属のみのカウントへ
- **修正2(app.js・本命)**: **契約枠が人気低下で無言で縮む**。枠は「契約枠拡大!」ポップアップ(25/50/70/1位の一方向解放)として演出されているのに、実装は毎週orgPopからターゲットを再計算して**下方向にも代入**していた — 合法に抱えた12名が枠16→10への縮小で毎週「キャップ超過」を浴びる(fixture走破S1W46で発覚)。縮小の通知手段はゲームに存在せず、設計意図は解放型と判断して**ラチェット化(拡大のみ)**: 週次milestone(App.checkRosterCapMilestones)/ロード時修復の両方を `max(現枠, ターゲット)` へ。キャップ制導入前セーブの一度きり移行にも「既所属数を下回らない」床を追加
- 発覚経路メモ: 走破fixture(12名/枠16/orgPop80)でランダムプレイが人気を落とし枠縮小→D1検出。auto-simはロスター運用が穏やかで一度も踏まない型(UI走破ハーネスの実績)
- バージョン1.32→1.33、push+配布梱包(package-release.ps1+verify)は本エントリ直後に実施
...
- **内容**: 3タブ(決裁/スカウト/レンタル)+タブ連動HUD+壁/机ジオラマ+交渉/解雇面談の間借りモードという全体構造、決裁書類カード(196×260・±3°決定論回転・朱印残置)、招聘市場常設ミニパネル(P3-3)、FA履歴書(アッパー172×258=2:3梯子XL)、レンタルミニ契約書、mdl-a様式のモーダル群(対象選択→ボーナス4案/休暇週数の2段遷移・招聘2段・過程イベント)の構成要素と状態バリエーションを全部現物から拾って記述
- **現状の負債も記録**: ハードコード色の残存(ステバー5色・レンタル費用ピンク等)、HUDとトップバーの情報重複、スカウトタブとscoutEvent画面の役割分担 — 未決事項欄に列挙(改修時の宿題)
...
- **G15(二重計上解消)**: 招聘コーチ由来のcoachMulからスタイル一致+0.08を除去(一致ボーナスはcalcInviteMultに一本化・gMultは残す)。計測: 非一致=ビット一致(7.27025→7.27025)/一致=-6.32%(織り込み済み・予測-6〜8%内)。C<B<A単調性維持
- **P3-4**: calcInviteMult分岐順修正 — 全36スタイルセルの機械比較で変化は**Allround×Allroundの0.05→0.08のみ**(43名に「ピッタリ」開通)。招聘A級解禁を orgPop≥55 || coachSlots≥4 へ(雇用側との通貨整合。C/B不変)
- **P3-5**: COACHING_COMPAT_ARCHETYPE新設(personality='normal'のときだけ参照・standardは±0)。normal33名中27名に相性が発生、非normalの判定は変化0
- **P3-2 重点ステ指導**: 招聘時に重点ステを指定→期間中その練習選択率×1.25。実測: TE指定でシェア24.99%→29.39%(理論値29.41%)。特化×1.40と同ステは非累積を4ステ全てで証明。AI招聘は従来どおり
- **P3-1 指名リクエスト**: 秘書に「スタイル/格」1軸(⚡0・費用0・顔ぶれの期に1回)。充足抽選は専用ソルト0x1CB3 — **リクエスト不使用の市場ロールは12,000回で不一致0**(ビット一致)。不在は正直報告(B級@pop20=500/500不在、ご都合補充なし)。--care 40季: 依頼195/充足172/不在22
- **P3-3**: 社長室決裁タブに招聘市場の常設ミニパネル(候補の顔+格+タイプ+スタイル+「あと◯週」+依頼UI)+入れ替わり前週のティッカー予告1行(乱数不消費の確定枠)
- **実装判断2件(Fable裁定)**: ①依頼の単位を暦の四半期でなく**「実際に見えている顔ぶれの期」**に(市場の入れ替わりは暦より1週遅れて見える既存挙動があり、暦基準だと未回答の依頼が28/32件silent上書きされる実測) — 承認。1週遅れ自体の是正は0x1CB1消費順に触るため別タスク扱い ②社長室の画面仕様書(03-screens)が元から存在しない — 追加パネルにつき今回は許容、仕様書新規作成は別タスク化
- **検証**: npm test 256/256(worktree+マージ後main) / walkthrough Issues:0 / auto-sim 40季42両モードALL CLEAR / **ケアなし指紋変化はG15単独由来と段階ビルドで証明**(G15適用時のみ変化、以降HEAD まで不変。招聘式はケアなし40季で20回しか走らない)
- specs: shachoshitsu v1.2追記にP3節を追加。実機確認はバックログ§P3へ。**これでcare-rework2の実装はP0〜P3全完了** — 残: Keisuke実機確認(バックログ)/specs diff確認/P4経済は全工程後に再検討(08-30裁定)

## care-rework2 P2-G「起用を約束する」実装完了 — task-101(Opus実装/Fable検算・マージ)（2026-08-30）

bold(強気)24名の「言葉が効かない」個性(声かけ×0.70=💤確定)を保ったまま、試合で応えるケアチャンネルを新設。Keisuke承認済みセリフ63本を焼き込み(文字63/63一致。草案の「」はMarkdown会話マーカーにつき吹き出し慣例に従い除去)。

- **実装(7コミット・Fable diff全文レビュー+検算マージ)**: DECISION_DOCS.pledge(⚡1/費用0/CD16週/机に並ばない)/`G.pledge`同時1件/判定はtickWeekの精算直後にsettlePledge(純関数・読み取り専用カード走査)。履行=+8×1.3(PLEDGE_BOLD_MULT、boldで初の実効1.0超)+スランプ/モチベ中momentum+8/破約=-6固定(OVR傾斜・finalMult非適用=「-6以内」保証。実測max5.97)/失効=判定機会なし12週で静か(退団時も罰なし)。UI: 選手ポップアップの金色バー(押せない状態は理由ラベル)/編成画面の予約印(ロックなし・特別興行週は持ち越し表示)/結果は興行後ポップアップ直列化キューに1枚
- **実装者の重要な指摘2件(Fable承認)**: ①指示書の「メイン=最終試合」はコードベース規約(メイン=showCard[0]先頭)と矛盾 — 規約に従い実装(指示書どおりだと約束が正しく判定されないところだった。指示書に訂正注記済み) ②PLEDGE_LINESをtools/extract-dialogue.jsのレジストリに登録(ワークブック往復から不可視になるGLIMPSE_B型の罠を未然に防止)
- **プローブが実バグを捕獲**: `_pendingPledgeResult`が消費されない経路で残り続け、同じ結果ポップアップが毎週再表示される(初回計測で23約束→破約1,673件の矛盾から発見)。1週限りのtransientとして修正、件数が完全一致(23=履行6+破約17)
- **検証**: npm test **256/256**(pledge系テスト3本新設: channel/lines-guard/tickweek-wiring) / auto-sim 40季42 --careあり/なし両ALL CLEAR / **ケアなしの意味指紋がベースラインとバイト一致**(未使用時の挙動不変を証明) / 履行・破約は3シードで発火確認(42:6/17、7:11/50、99:20/59)。失効は通常興行が約2.8週毎のため自然発生が構造的にほぼ不可能 — tickWeek実走の決定論テストで経路を証明 / walkthrough 310操作 Issues:0 / ui-check 7/7
- **発見(要Keisuke確認)**: 旗揚げ時ロスター5名にboldが0名 — このチャンネルはbold加入後に初めて開く。意図確認はバックログへ
- specs: shachoshitsu v1.2追記にpledge節を追加。実機確認7項目はバックログ§P2-Gへ
- 残: task-102(P3招聘市場・実行中)

## care-rework2 P2-A〜F「書類の役割再定義」実装完了 — task-100(Opus実装/Fable検算・マージ)+P3設計v0.1+約束セリフ承認（2026-08-30）

Keisuke「どんどん進めて止まらないで」を受け、P2設計→task-100実装→マージ→P3設計まで同日進行。設計の正: `docs/care-rework2-p2-design-v0.1.md` / `docs/care-rework2-p3-design-v0.1.md`。

- **P2設計(Fable)**: 各書類に固有ニッチを1つずつ — 休暇=寿命(wear)ケア/慰労会=士気の一手/特別治療=長期離脱(総週数10+)の劇的救済/メディア=人気純化/ボーナス=±22%の揺らぎ/倍率テーブル整理(composed・polite行新設、special_treatment死に列削除)。⚡コストは全書類不変。設計前にExploreで現行実数を全棚卸し(DECISION_DOCS/wearの桁/怪我週数分布/士気力学/⚡経済)
- **task-100実装(Opus隔離worktree・6コミット・Fable diff全文レビュー+検算マージ)**: 上記6件+副産物2件 — ①team書類のCD判定が実はdoc.cooldownを見ておらず常に1週相当だった問題を修正 ②injury.totalWeeksを全発生箇所で記録(旧セーブはweeksLeftフォールバック=対象を取りこぼす安全側)。特別治療のポップアップ経路⚡0/200万の不整合も⚡1/500万に統一
- **検証**: npm test 253/253(worktree+マージ後main) / auto-sim 40季42×4本(HEAD/base×care/なし)全ALL CLEAR / **受け入れゲート: --careのmorale平均61.80(58〜64内、P1表示較正の前提維持)** / **引退年齢中央値: base無27.0/HEAD無27.0/base有28.0/HEAD有27.0=±1季以内**(P2-A不変条件充足。ケアなし2本はSource行以外の全出力が一致) / walkthrough PASS Issues:0
- **申し送りの裁定**: 怪我ポップアップの治療ボタンは呼び出し元が存在しない死に導線と判明(エンジン側の整合のみ修正・ボタン蘇生は見送り) / B1イベントの「特別治療(-200万)」は初期離脱週数を決める別物なので据え置き(文言分離は将来課題)
- **P3設計v0.1(Fable・同日)**: 指名リクエスト(秘書・四半期1回・不在は正直報告)/重点ステ指導(×1.25、コーチデータ不変で13セル問題を解消・特化×1.40の上位は維持)/市場常設パネル+入れ替え予告/Allround選手43名の一致経路開通(分岐順修正)/normal33名の相性のarchetype副軸/**G15新規台帳=スタイル一致ボーナスの二重計上バグ発見**(calcInviteMultとcoachMulの両方に+0.08)。実装はtask-102(101の後)
- **起用約束セリフ63本**: Opus起草→**Keisuke全文承認(同日「とてもいいですね」)**。クール×強気の外挿セル含め確定版。task-101で無改変焼き込み
- specs: shachoshitsu-spec-v1.0にv1.2現物合わせ追記(P2の全値)。**Keisukeのspecs diff確認待ち**
- 実機確認6項目はバックログ§care-rework2 P2へ。残: task-101(起用約束実装・実行中)→task-102(P3)

## care-rework2 P1「気づきの再建」実装完了 — 較正実測+task-99(Opus実装/Fable検算・マージ084d659)（2026-08-30）

Keisuke裁定「紙上レビューでは分からない、一旦実装して現物で判断」を受け、P1設計v0.1の3論点を推奨デフォルト(常設アイコン方式/⚡n/6数字表示/L2日常化の温度感)で実装し実機判断へ回した。**同日、P2・P3も実施確定**(P1の後この順)、P4経済はP1〜P3完了後に改めて検討(大掛かりなため)と裁定。

- **P1-1閾値の較正実測(Fable)**: 計装auto-sim(--careあり/なし各40季×シード42,7919、週次morale分布+表示ノイズ畳み込み)で確定 — レベル閾値 **≤24/≤41/≤57/≤65/66+**、表示ノイズ **±10→±4**(帯幅20→10に伴い「ノイズ≒帯幅の半分」比を維持)、空気ログgood閾値 **70→58**(=L4開始と一致)。**設計docの当初目標「ケアなしL4+<10%」は達成不能と判明し改定**(実効レンジは想定41〜62でなく0〜70。士気の第一因子は興行MQで、ケアなしでも好調期は正当に上がる)。結果: ケアなし=中央値33がL2「重い」・L3占有80.7%→22〜35% / ケアあり=L4常態65〜72%・L5時々15〜19%
- **task-99実装(Opus隔離worktree・8コミット・Fable diff全文レビュー+検算でfast-forwardマージ)**: P1-1段階較正(G12) / P1-4 ⚡n/6常設+満タン明滅+社長室navドット+机カード朱印残し(getAvailableDocsにopts.includeWeekUsed、既定false=従来と同一)+選手CD「あと n週」 / P1-2 信頼40-55帯🌫サイン(ヒステリシス40-55点灯/57消灯、記憶はui-render内Mapのみでセーブ・エンジン不関与) / P1-3 空気ログ復活(G5恒偽条件`!isShowWeek&&week%2===0`→確率30%、LOCKER_AIR_TEXTS14本が実装以来初めて発火) / P1-5 遅延発現の集約トースト+最終週の無通知解消+週次レポート帰結1行
- **申し送り2の裁定(Fable)**: navドットが仕様どおりだと点灯率91%(ケアなし)で死に信号化 — 真因はrelationship_repairの慢性条件。**案(b)未読方式を採用**(その週に社長室を開くと消灯・翌週案件があれば再点灯。新聞バッジ`_bigNewsUnread`と同系統、マーカーは`G._shachoshitsuSeenWeek`でUI層のみが読む・旧セーブは未読フォールバック)
- **検証**: npm test 253/253(worktree+マージ後main両方) / auto-sim 40季42 --careあり/なし両ALL CLEAR+**平均trust/moraleが修正前後で小数2桁まで完全一致**(なし74.74/34.28・あり86.91/61.12=表示層のみの証明)。care-off意味指紋不変、care-onの指紋変化は_pendingTrustReveals(isFinal付与)のみと切り分け済み(除外ハッシュで前後一致10e864ba) / UI走破 PASS Issues:0(社長室画面を実走) / ui-check 7項目○
- **specs**: `specs/care-visibility-spec-v1.0.md` 新設(+INDEX追記)、`specs/poach-trust-spec-v1.0.md` §3.2/§3.3を現物合わせ改訂(空気ログ30%化・good=58)。**Keisukeのspecs diff確認待ち**
- **新規文言**(数値語・内部変数名なし。実機確認と同時にレビュー): 🌫ホバー3種(「最近、目を合わせる回数が減った気がする」等)/集約トースト2種(「合宿の手応えが出てきた(N名)」「外部コーチの指導が実を結び始めた(N名)」)/⚡ツールチップ/CD文言。全文は task-99-report.md §2
- **副次調査**: docs/stat-contribution-report.md の恒常git dirtの正体は「npm testのstat-contribution-test.jsが毎回レポートを再生成しており、08-20試合式変更後の正しい数値が未コミットのままだった」こと(シードは固定で決定論的)。今回の再生成を本コミットに含めて解消
- 実機確認9項目はバックログ§care-rework2 P1へ記帳。残課題: P2書類の役割再定義(次工程・設計から)/⚡配分でケア系が回らない件(B5合流、P2)/合宿公称値の再設計判断(P2)
...
- **手法**: 理論天井の静的解析(127テンプレ) / 理想条件プロジェクション(127名×30シード) / auto-sim計装版によるフル環境シム(現行main 40季7919+100季42、**pre-P3世界(WM_SOURCE_REF=64f092e)との同シードA/B**)。auto-sim本体は未変更、計装コピーはscratchpad
- **結論**: 観察は本物。ただし成長式は尖りに中立〜有利(現行mainフル環境で尖りpotのP(100+)48% > フラット32%、深町peak115/高津6/6が100+)。原因は外側の5つ:
  1. **【バグ】AI団体所属選手のスランプ/モチベ喪失が永久回復しない** — 回復判定tickSlumpPassive/tickMotivationLossPassive(management.js:12537/12558)がプレイヤーロスター週次にしか無く、AI側は発生(10710-10721)のみ。AI高pot若手の37〜40%が凍結(meanPeak 97.7→88.2)。40季7919の高津はこれでpeak73凍結(モチベ喪失443週)。08-01裁定「AI/プレイヤー成長対称」違反
  2. **【バグ】AIドラフト落札新人がcareerStage:'prospect'のまま**(ui-common.js:5975/6002のnormFighterが変換しない。100季で48週以上残留361件、careerSeasons不計上)
  3. **【構造】cap係数の団体格差** — プレイヤー入手[0.55,0.80] vs S団体エリート枠[0.70-0.75,0.80]。100季の100+94件=org_s72/org_a21/org_b1/**プレイヤー0**(95+もゼロ)
  4. **【歴史】既存セーブはP3ソート化以前の旧capのまま**(31.5%最強ステ入替・尖りSD×0.87)。同シードA/Bでpre-P3の100+コホートは尖りpot割合17%(現行37%)・実現SD13.2(現行16.5)と明確にフラット寄り=体感の直接原因
  5. **【構造】MNは練習で伸びない**(キャリア中央値+7〜14)+OVR=5ステ平均 → pot最強がMNの高津型はソート最大係数が死蔵(理論天井-5.8pt)
- **修正候補**(着手はKeisuke判断): Fix-A=AI週次に回復tick追加(要再較正) / Fix-B=normFighterにcareerStage:'active' / 設計判断=プレイヤー側の天井引き上げ手段・MN成長経路・既存セーブの扱い
- 残課題: Fix-A/Bの実装判断待ち。P3b(端数持ち越し)マージ時は本調査の再計測が必要
...
## 数値オーバーホールP3b完了: calcGrowthのceil 1pt下限を端数持ち越しへ是正+2軸補償再較正（2026-08-20・Fable）

care-rework2 task-98裁定Bの申し送り(cap近傍0.85帯で招聘倍率の差がceilに潰される)を解消。**「rawGain 0.4でも+1pt」の隠しインフレを廃止し、成長の量子化を「端数はいつか必ず実る」持ち越し方式へ**。

- **実装**: ①`calcGrowth`はceilを廃止して0.1刻みの小数を返す(rng消費順序は不変) ②新設`Engine.growth.settleGrowthFraction`(純関数)で、呼び出し元3箇所(AI週処理processAIWeek/追い込み/通常練習)が事後倍率を掛けた後に整数pt+持ち越し端数へ分解、端数は選手の`_growthFrac`へ蓄積。**事後倍率(孤立0.7/警告0.9等)も従来のroundに食われなくなった**(growth=1×0.7がround→1で無効化されていた) ③セーブ互換: 未定義→0扱い、cap衝突で超過端数は破棄、`validateGameState`がNaN・範囲外(frac≥1)を警告つきで0へ自動修正 ④計測器`test/growth-intensive-projection.js`も本番と同じ持ち越しに更新+`--baseLearning`オプション追加
- **重要な発見: baseLearning単独では補償不能(タスク前提の修正)**。ceilは実質「壁際限定の成長フロア」で、γ1.3(v2.1)はフロア込みで較正されていた。フロア撤去後にblだけ上げて年間成長を合わせると**カンスト率が全bl候補で0%へ崩壊**(r^1.3の尻尾が遅すぎてキャリア内に98%へ届かない)。補償は2軸再フィットで**baseLearning 3.0→4.5 / brakeGamma 1.3→1.1**を採用。γ<1.0は開眼(γ→1.0解放)のブースト方向を反転させるため不可、1.0超を維持
- **較正(3シード×127名projection、γ1.3/bl3.0の凍結beforeとペアド比較)**: 追い込み連打カンスト率(27歳4stat98%+) 35.7%→36.7%(+2.8%) / キャリア年間成長 I -0.4%・I2 -1.3%・N -4.3% / 3年目中央値 +3〜4%。**25季フルシミュ**(growth-baseline-measure、seed1001、WM_SOURCE_REF=64f092eペアド): 総成長+2.7% / AI-SトップOVR帯 105.9→102.8(-2.9%) / 到達率中央値86.9→88.7%。全て±10%内。**既知のトレードオフ**: 純粋放置(projection N帯)の壁際到達だけは旧フロアのregressiveな下駄(低成長ほど大きい切り上げ)を滑らかな再スケールでは再現できず低下(エースOVR100+ 17.7→7.1%)。実運用の混合レジーム(フルシミュ)では枠内
- **検証**: `test/growth-frac-carry-test.js`新設(QUICK登録) — F1保存則(0.3×10=ちょうど3pt/浮動小数ノイズ/cap衝突)/F2小数返却/T1招聘倍率単調性(1.00<1.25<1.35<1.53)/**T3 cap近傍0.85帯で×1.25と×1.53の差が成長に出る(task-98未達項の解消)**/F3 validateGameState自動修正、全PASS。npm run test:quick 110/110。kaigan-invariants単体PASS(開眼γ解放はγ1.1でも有効)。auto-sim 40季42 **--careあり/なし両方ALL CLEAR**(ケア効果は方向維持: trust 74.9→87.9/morale 35.6→59.8)+採用値の最終確認100季1本ALL CLEAR
- specs: growth-system-spec-v2.2.mdに**v2.2.3行**+§1核心式を現行化(小数返却+settleGrowthFraction適用)。提案書§3-P3にP3b完了記帳。**Keisukeのspecs diff確認待ち**
- 残: P2/P3(trainCapソート化)は本作業のスコープ外(別セッションで進行、worktree分岐点64f092eには未取り込み — マージ時にworklog/roadmap/提案書の記帳を統合すること)。実機確認は「成長ペースの体感」(特に壁際で+1が出る頻度が下がり、数週おきにまとまって実る感触に変わる)をバックログへ
...
- **較正(scratchpadプロト・凍結ベースラインと同一シード同一乱数列でペアド比較)**: べき平均pグリッド{1,3,4,5}→**p=4採用**(p=5は低OVR帯で60%超過熱)。MN補強グリッド→**defMntScale 0.08 / pinAttemptMntPenalty 0.24採用**(0.09/0.28は副作用大、なしはMN@100が47.3で未達)。pは同格アンカー・格差・スタイル均衡に一切影響しない純粋な尖りプレミアム専用レバーであることを実測確認
- **実装(src/data.js ENG + src/match-engine.js)**: ①命中差分化 `hitBase+(TE_atk−SP_def)×0.18`(tecHitBonus/spdDodgeBonus廃止) ②カウンター差分化 `1.5+(TE_def−SP_atk)×0.06`+上限25(counterTecScale/counterSpdPenalty廃止) ③kickout/guEscapeのsoft-knee(MN80まで現行・80超漸近、上限0.60/0.52、kickoutKnee/guEscapeKnee定数) ④OVR比ダメージ補正をbattlePower(べき平均p=powerMeanP=4)参照へ(表示OVR/MQ天井/開幕大技判定は算術平均のまま) ⑤MN価値補強。タッグは判定関数共用で①②③⑤自動波及(④はタッグにOVR比補正なし)
- **検証**: balance-baseline比較=逸脱36件が**全てspikeGridの上方向(意図した尖りプレミアム発現)**、不変条件系(スタイル均衡49.7-50.3/格差+10=76.4%/同格アンカー/決着分布)は全て許容内。唯一の織り込み済み副作用=脱出0.051→0.044回/試合(MN補強でダメージ微減のため)。**+24尖り@100帯: PW53.9/SP54.8/TE53.4/ST53.6/MN48.0**(目標53-58、MN48-53達成)。+40級@100帯は56.6-58.9で60%未満。P1後の姿を新ベースラインとして凍結し直し。auto-sim 20季42 **ALL CLEAR**
- **既知の記録**: @60帯の+40級(相対67%の合成極端配分)は67-72.5%まで伸びる — 実在キャラの尖りは相対30%以下でありゲーム内では到達しない外挿域。べき平均はスケール不変=相対尖りに反応するため「低OVR帯の一芸ほど大きく光る」(仕様として意図)。@120帯のST+24のみ50.6と控えめ(損はしない)
- specs: battle-engine-spec-v4.2.mdに**v6.0行**を追記(式の現在形はv6.0行が正)+INDEX.md更新。**Keisukeのspecs diff確認待ち**
- 残: P2(RNG規約/resolveMatchCardデッドコード削除/eff残骸撤去/共有ストリーム監査) → P3(trainCapソート化+care-rework2裁定Bのceil量子化対応) → P4受け入れ。タッグ専用の較正計測はP1b(必要なら)

## 数値オーバーホールP0完了: 計測方式バグ発見+真のベースライン凍結（2026-08-20・Fable）

numeric-overhaul(ゴー確定・55%中心・P3ソート方式・eff撤去確定)のP0を実施。**最大の成果は較正に入る前に計測装置自体のバグを発見したこと**。

- **計測方式バグ**: 1本のrngストリームを共有して大量試合を回すと、Engine.rng(xorshift128+の32bit移植、シフト定数23/17/26は32bitでは品質未検証)の長距離相関で**勝率がシードにより最大35pt動く**(5シード×8000試合で実証: 方式1スプレッド5〜35pt、本体準拠方式0.9〜2.1pt)。**08-19調査レポートの試合系数値はこれに汚染されていた**→レポート冒頭に訂正ブロック追記済み
- **本体は無事**: 全試合経路(通常/AI興行/対抗戦/王座戦/JT/タッグ)が試合ごと`create(derive(...))`を全数確認。唯一の共有rng受け`relationships.js resolveMatchCard`は呼び出し元ゼロのデッドコード(P2で削除)
- **真のベースライン**(`test/balance-baseline.js`新設・`test/baseline/balance-baseline.json`に凍結・別シードで逸脱なし確認): 同格アンカー=ターン14.36/カウンター0.66/脱出0.05/MQ79.5/時間切れ1.3%。格差カーブ単調(+5=62.3%/+10=75.1%/+20=91.7%)。**スタイルは完全均衡(49.7〜50.3%)→旧P2の技プール較正は不要と判明**。尖りはPW/SP/TE/STが全帯全幅で48.7〜52.7%とほぼ中立、**MNだけ本物の罰**(+24で42〜45%、+40で38〜41%)
- **P1の課題が確定**: ①尖り(PW/SP/TE/ST)に+3〜5ptの報酬を作り55%中心へ ②MNの罰を解消し48〜53%へ ③TE/MN飽和の構造是正(勝率では相殺されて見えないが「盛った分が効かない」は実在)
- 提案書更新: P0完了記帳/P2を「RNG規約と残骸整理」に差し替え/計測規約を原則5に追加/不変条件を凍結値ベースに更新
- 検証: src/無変更(test/追加のみ)。auto-sim対象外。manifest対象外(配布物なし)

## task-98マージ: care-rework2 P0完了 — 丸め修正+--care計装+二重適用是正（2026-08-20・Opus実装/Fable検算・マージa4bd2f2）

Opusが隔離worktreeで実装(前セッション跨ぎで再開)、Fableがdiff全文レビュー+不変条件検算の上mainへマージ。**これでcare-rework2 P0は全項目完了**。詳細は `docs/codex-tasks/task-98-report.md`。

- **実装1(丸めの死の解消)**: 招聘/合宿倍率(`getTrainerMult`)を `calcGrowth` 内の量子化前(baseGain乗算列)へ移設、呼び出し側3箇所(AI/intensive/practice)から除去。**T1単調性PASS**(1.00→4.000 / 1.25→4.296 / 1.35→4.625 / 1.53→5.184、4週期待成長)。**I1ビット一致PASS**(倍率1.0で25,920ケース一致+統制auto-sim 20季指紋一致)。specはv2.2.2として反映
- **重要発見(Fable裁定A)**: 旧実装は0倍だけでなく**2倍も作っていた**(`round(1.5)=2`で倍率1.45以上が実効2.0倍化)。合宿×1.5の実効が4.000→2.697(-33%)と公称値へ正されるため、不変条件「現状から悪化しない」は「現状=バグ値」につき**「公称×1.5に一致」へ改定して採用**。体感低下は実機確認バックログへ、公称値の再設計はP2の判断材料
- **裁定B**: T3(cap近傍0.85帯でも倍率差が出る)は未達のまま受理。原因は`ceil`の1pt下限=第2の量子化器で、端数持ち越し/soft-kneeは**数値オーバーホール(ゴー確定済み)の管轄へ申し送り**。「壁の前では招聘は効かず、限界突破系だけが壁を壊せる」現状の構造は物語として維持
- **実装2(auto-sim --care)**: 毎週決定的規則でケア決裁を自動実行。40季42シードでケアあり/なし: trust 75.6→86.8 / morale 38.3→61.2 / OVR +0.47。⚡配分で招聘0.15回/季しか回らない件はP2申し送り(既存論点B5と合流)
- **実装3(G13)**: `executeShow`内のtrust/morale適用を除去しprocessSettlementの1回に統一(実プレイは元から1回・app.jsはexecuteShow不使用を全数確認済み)。auto-simの士気が実プレイと一致するようになった
- **統合検証(Fable)**: マージ後main(ブラケット棚卸しa270aa6込み)で auto-sim 20季42 **ALL CLEAR**。指紋変化(8ab84fe7→5886d249)は切り分け済み: a270aa6は指紋中立(43cc2cc時点の8ab84fe7一致を一時worktreeで実測)、変化は実装1+実装3の意図した挙動変化のみ。npm test 252/252(worktree内)
- CLAUDE.mdの手動実行例に--careを追記(今後のケア系・成長系較正は--careあり/なし両方で測る)。実機確認バックログにP0の5項目を追加
...
## task-97マージ: care-rework2 P0機械修理8件（2026-08-20・Opus実装/Fable検算・マージ8668296）

Opusが隔離worktreeで実装(9コミット、51挿入/11削除)、Fableがdiff全文レビュー+不変条件検算の上mainへマージ。G1偏りシャッフル→Fisher-Yates(10,000シード実測: 格差3.86倍→1.23倍・χ²2706→38.5) / G2永久出禁→1回休み(40シードシナリオでQ2除外・Q3出現を確認) / G3自動継続デフォルトOFF+消化力逓減の対称化 / G4休暇中の欠場trustペナルティ除外(onLeaveフィールド実在をFable検証) / G6 shy行追加(設計値一字一致をFable検証) / G8エラーフォールスルー封鎖(offseason_locked+汎用防壁、pair系3コードの素通りも同時解消) / G10招聘UI事前チェック2件 / G11トースト文言。auto-sim 20季ALL CLEAR。詳細は `docs/codex-tasks/task-97-report.md`。

**申し送り3件へのFable裁定**: ①G1の±15%閾値は統計的に不成立(1.3σ)→χ²判定を正とする(承認) ②G4でsameOrgActiveRoster(孤立判定の母数)は不変条件優先で未変更→**採用**。休暇を母数から外すかはP2役割再定義で扱う ③G8のpair系エラー3件は汎用防壁で対応→**採用**(到達は異常系のみ、専用文言不要)。UI走破テストはマージ後にFableが実行 → **実行済みPASS(2026-08-20、Session Fクリーンアップ統合後のmain=e2df748相当で325操作走破・issues 0)**。
...
- **`11d1dc4`（2026-07-22）: `Engine.specialEventFinance` 新設** — 5大特別大会に固定興行収入を導入（standard 7,000万 / premium 1億1,000万、equal 30%+出場枠比例70%で参加団体へ分配、management.js:26997〜）。旧・選手単位PPV出演料を置き換え。**資金インフレの主因の一つ**（ケアコストの相対的無意味化に寄与、詳細は docs/care-rework2-plan-v0.1.md §1.2）
- **`13aa69e`（2026-08-05）: 入団成熟度引き上げ+遅咲き廃止+季末wearの当季試合数連動化** — ①`getEntryMaturityRatio` を17歳0.75〜23歳+1.00へ引き上げ（旧0.40-0.70帯から大幅増、入団時伸びしろ約2割減） ②`遅咲き`特性を廃止（早熟/晩成のみに） ③季末wearの試合量項を「キャリア平均≥40で+3」から当季試合数連動（≤10: −1 / 19〜24: +2 / ≥25: +4）へ変更。**specs/growth-system-spec-v2.2 に v2.2.1 として現物合わせ追記済み（同日）**

## D-3 徹底調査+care-rework2計画v0.1（2026-08-20・Fable統合/Opus5系統並列調査・コード変更なし）

D-3(ケアアクションが使われない/招聘コーチがニーズに合わない/高津・出羽が高確率で不機嫌)をOpusエージェント5体の並列調査(仕様棚卸し/放置実害計測/招聘市場定量化/バランス変遷史/不機嫌現象)で徹底調査し、Fableが統合して**調査+計画v0.1**を作成した。正は `docs/care-rework2-plan-v0.1.md`。src/は無変更(計測は全てscratchpadコピーで実施、auto-sim改によるケアon/off比較 3シード×20季×2モード等)。

- **主要な確定事実**: ①ケア0回で20季回しても平均信頼76.5・突然退団0.33件(信頼はブッキングで平衡84.6に飽和し書類が対象を持てない)。一方ケアon/offで**OVR+2.25/20季(S20で+4.4)・士気+14・モチベ喪失半減の実害が実在**——罰は毎週発火しているのに道場テキストは80.7%の週で「特に変わった様子はない」(**罰が見えない**) ②犯人は3系統複合: 信頼可視性全廃(07-21+08-13)/成長式の丸めで**倍率1.45未満の招聘は成長0pt**(γ1.3+08-05成熟度で死に帯86%)/特別大会収益新設(07-22)で金の拘束力消滅。信頼エンジンは7/7から無変更 ③招聘市場はバグ2(偏りシャッフル3.8倍格差・永久出禁)+構造欠陥(ニーズ完全無視・ピッタリ不在34%・自動継続デフォルトONで市場恒久ロック) ④高津・出羽の不機嫌は発生確率でなく**bold設計の三点セット**(怒りの語彙=意図的個性/encourage×0.70で💤100%確定/最安ボーナス100%侮辱)+高津固有のG2恒常発火41.5%(te47+晩成、全キャラ2位)
- **バグ・死にコード台帳14件**(G1〜G14、計画doc§1.5): LOCKER_AIR_TEXTS14本デッドコード(発火条件の週偶奇自己矛盾)/休暇辞令の信頼収支赤字/shy5名テーブル欠落/未記帳コミット2本(11d1dc4特別大会収益・13aa69e入団成熟度)等
- **計画の骨子**: P0修理と計装(丸め局所修正・auto-simに--careモード追加)→P1気づきの再建(可視化・数値は見せない)→P2書類の役割再定義(ニッチ化+性格別「効くチャンネル」)→P3招聘市場(指名リクエスト=秘書演出+母集団穴埋め)→P4経済は切り出し推奨。計6〜8.5セッション級。数値目標には不変条件を対で記載
- **同日ゴー確定(Keisuke裁定)**: §5全8点を推奨どおり承認、#7高津のみ「キャラ能力値改定側で解消」。裁定を計画docに記帳し**P0着手**: ①task-97(機械修理G1/G2/G3/G4/G6/G8/G10/G11文言。shy行はFable設計値を指示書に確定)+task-98(丸め修正=trainerMultをcalcGrowth内rawGain段階へ移設+auto-sim --careモード+G13二重適用是正。不変条件I1〜I5対書き)をCodex委譲(97→98の順) ②G14記帳完了: worklogに未記帳2コミット(11d1dc4/13aa69e)を遡及記帳+growth-system-spec-v2.2へ**v2.2.1現物合わせ追記**(遅咲き廃止/入団成熟度/季末wear当季試合数連動)→**specs diffはKeisuke確認待ち** ③G7(関係修復セリフ欠落)はOpus起草→**Keisuke全文レビューOK→data.js配線完了(同日)**。宣言リテラル内挿入(ブラケット代入不使用)、全127名×success/failの解決テスト254/254通過。草案の正: docs/dialogue/relationship-repair-reactions-draft-v0.1.md ④**実行体制変更(Keisuke指示)**: task-97/98はCodexでなく**Opusが隔離worktreeで実装**し、Fableがdiffレビュー+不変条件検算→mainマージ(97→98の順)。次: task-97のマージレビュー→task-98→P1可視化

2026-08-20のDiscordでのテスターTOMORIの実感（「最近はトレーニングのケアアクを使ってない」「Wコーチは毎回こっちのニーズに合わない感」）とKeisukeの同意（「再設計が必要かも」「コストも格安」「わずらわしさもあるかも」「使わなくても問題ないゲームバランスに、いつの間にか変わってるのかも」「せっかくつけるなら納得感が欲しい」）を受け、**手直し方針の検討から始める検討項目**としてロードマップに登録した。

- 記帳先: `docs/game-system-roadmap.md` の「要設計相談（社長室コマンド系）」に **D-3** として追加（D-1ボーナス再設計/D-2招聘制リデザインの続き番号。care-rework v0.1 の再々設計になる可能性を含む）
- 対象: 社長室の決裁書類系ケア（bonus/refresh_leave/special_treatment/party/trainer=外部コーチ招聘/camp/media + 声かけencourage）
- 論点3つ: ①使われない原因の切り分け（効果/必要性の低下/操作コスト） ②書類ごとの役割再定義 ③招聘候補の抽選をニーズに寄せるか
- コード変更なし（docsのみ）。auto-sim対象外


平均型vs尖り調査を受けたKeisuke裁定(①TE/MN飽和は許容しない ②尖りのある選手がむしろ強い世界に ③含意1・2・3全部やる ④能力値レンジが当初設計から変わった、部分修正でなく「数字の働き方」を作り直し覚悟で全体見直し)に対する設計提案。正は `docs/numeric-overhaul-proposal-v0.1.md`。

- **追加計測3本を先に実施**(エンジン直叩き・シード20260819): (A)技プール診断 — Striker/Brawlerの威力プール(期待d)は他と同等で、敗因は式でなく創発挙動。同ステなのにStrikerの被カウンター率だけ相手の3倍(8.93% vs 3.08%)という**式からは出ない非対称**を観測。計装なしで技データを触るのは危険と判断 (B)差分化+soft-kneeのプロトをエンジンに実パッチして計測 — TE+24が全帯50%超に単調化・橘実配分がOVR6差を配分で相殺(52.7%)という良い方向と、PW+24暴騰65%・時間切れ消滅などの副作用が両方出た。**式1本の差し替えで系全体が連動=個別パッチ不可、全体較正必須の実証** (C)trainCap相関化グリッド — ソート方式(係数をpot順位に再割当)が保存率100%・分布完全維持・尖りSD比1.28で最有力
- **提案の骨子**: 新数値原則4つ(差分/比率参照・単調性保証=soft-knee・同格アンカー・尖りプレミアム) / P0計装+ベースライン凍結 → P1試合式再設計 → P2スタイル機構較正 → P3 trainCapソート化 → P4受け入れ。計5.5〜7.5セッション級
- 較正目標と不変条件は対で記載(+24尖り52〜58%・±40が60%超えない・格上OVR+10は71%±3pt・同格アンカー±10%等)
- 残: Keisukeのゴー判断3点(P0着手可否 / P3ソート方式採用 / 尖りプレミアムの強さ)

## 調査: 「平均型キャラは尖りキャラより有利か」計測 v0.1（2026-08-19・Fable・コード変更なし）

Keisukeの体感「オールラウンドの平均値が高いキャラばかり幅を利かせ、SP/TE尖りが活躍しない。偏りがあるとOVRが伸びにくい気がする」をエンジン直叩きで計測。成果物は `docs/flat-vs-spiky-stats-investigation-v0.1.md`(正・全実測値入り)。

- **成長面: 「偏りがあると伸びにくい」は否定**。cap合計・current合計を固定した統制実験(300シード)で、配分によらず成長速度・到達点は同一。実キャラ127名×30シードでも尖り度と成長量の相関 r=0.13。伸びを決めるのはcap総量と伸びしろ
- **試合面: 尖りへの報酬がなく、飽和帯の罰がある**。勝敗の主要因はOVR比ダメージ補正(=5ステ単純平均)。TEは命中率98クランプ(小中技は全員張り付き)・カウンター下限2張り付き、**MNはキックアウト率0.45クランプでMN90超が1ptも寄与しない死にステ**。ST(HP2.5/pt)/PW(0.27/pt)だけ線形満額。同OVRでも±40級尖り・2軸尖りはバランス型に42〜33%で負け越す
- **スタイル技プールに隠れた強弱**: 全員ステ100固定でもAllroundはStrikerに65.8%勝つ。平均勝率 Aerial 55.8 / Grappler 53.7 / Allround 53.4 / Submission 50.0 / Brawler 44.7 / **Striker 42.4**。「スピード・打撃系が活躍しない」体感の相当部分はここ
- **スクショ上位4名(倉見/大久保/観音崎/梅ヶ丘)は全員テンプレからstyle=Allround・pot均等設計**。「均等だから勝った」のではなく「上位才能が均等型に設計されている」。橘玲美は同OVR化すると倉見配分に55.2%で勝ち越す=配分無罪。下位なのはOVR素点と早熟(22歳で成長終了)のため
- 設計への含意4点(TE/MN飽和解消・Striker/Brawler技プール較正・OVR一元主義の認識・trainCap独立乱数の個性ぼやけ)はレポート§設計への含意。着手はKeisuke判断待ち
- 検証: src/ 無変更のためauto-sim不要。調査スクリプトはscratchpad実行(シード20260819、試合系は各ペア6000〜8000試合)

## 英語対応(ゲーム内言語設定)の棚卸し+見積もり v0.1（2026-08-19・Fable・実装未着手）

安定版到達を受けたKeisukeの英語設定意向(08-18)に対し、実装に入らず**棚卸しと見積もりだけ**を実施(裁定「まず見積もりだけ」)。成果物は `docs/i18n-english-plan-v0.1.md`(正)と `test/i18n-scan.js`(計測スキャナ・数値は再現可能)。

- **実測**: JSをトークナイズしコメント/文字列リテラルを分離して計測。翻訳対象の日本語は**約47.6万字/28,700本**(コメント内15.5万字は対象外として除外)。セリフ系30.6万字(64%)が最大の山。カテゴリ別・data.jsセクション別内訳は計画doc §1
- **決定4点(08-18)**: ①ゴールはまず見積もりまで ②品質担保=英語トーンバイブルKeisuke承認+LLM本文+機械検査+スポットチェック ③固有名詞=Claudeドラフト→Keisuke承認 ④既存セーブでもいつでも言語切替可(**内部キーは日本語のまま維持・表示時のみ翻訳**。キー英語化はセーブ互換破壊+最大のバグ源なので禁止)
- **方針**: 5層分割(ロジックキー/UIクローム/テンプレ/セリフ/固有名詞)。i18n憲章v1.0(docs/i18n-charter-v1.0.md)の原則を流用し、意図的逸脱は「ロジックキーの言語中立ID化をしない」の1点のみ。防御装備=JAゴールデンテスト/擬似ロケール/ラチェットガード/翻訳漏れログ/プレースホルダ機械検査
- **見積もり**: P1基盤〜P6仕上げで実働45〜75セッション級。コード3割・英文執筆7割。律速はP5セリフ(17,600本)
- **08-19追記: 二段構え方針(Keisuke)** — 「翻訳より先に、元データを翻訳しやすい構造に整える」。Stage A=翻訳可能化工事(P1+P2+P3a: コード内直書きセリフ/ナレーション/断片連結の分離、対象は実測8.1万字=全体の17%。残り83%は既にテーブル分離済み)を独立マイルストーン化。Stage B=翻訳。当面のゴー判断は「Stage Aをやるか」だけでよくなった
- 検証: src/ 無変更のためauto-sim対象外。manifest変更なし(docs+testのみ)
- 残: Stage A着手(**ゴー確定 2026-08-19「近いうちにやる予定」**。着手時はP1基盤から)
...
**修正(3層)**: ①`Engine.challengeRequest.dropStalePending`新設 — 発起人/相手の実在(+団体非解散)を検査し、不成立なら静かに取り下げ(不在データの説明は出さない。CD/クォータ不記録)。自浄は選手が退場しうる全遷移APIの4箇所: processWeekly冒頭(持ち越し早期returnより前)/**tickWeek末尾(validateGameState直前の正規化帯)**/retirement.commitRetirements(王座空位化と同列の退場者後始末)/advanceWeekラッパー(季末の契約満了・移籍) ②app.js handleChallengeRequestのnull分岐を無条件残置→dropStalePending経由のfail-openへ(actor実在時は従来どおり持ち越し)+`[WM][challenge-request]`警告 ③validateGameStateに直訴の参照整合性不変条件を追加(パイプライン外からの孤児持ち込みをauto-simで検出)。**後段2箇所はターン末Stopフックの検出で増設** — auto-simはadvanceWeek直後にもvalidateGameStateを通すため、季末(W49)の引退commit・契約満了がtickWeek外で作る孤児が不変条件に捕まった(5シード×20季で11件)。「遷移APIは整合状態で返す」を通す形で解消。
...
## 選択サーフェス統一の残件クローズ: カード編成ピッカー行2箇所（2026-08-14・Fable・Keisuke裁定）

③初回実走で裁定待ちにしていたカード編成ピッカー行(毎週使う画面のため操作感の変更は裁定必須だった)について、Keisuke裁定**「行と同じ=選択に揃える。ただし選手詳細は名前クリックで見られるように」**を受けて実施。

- **ui-render.js タッグスロットピッカー行(App.setTagSlotFighter)/シングル戦ピッカー行(_spSelectFighter)の2箇所**: portraitImg第4引数を除去(顔=素通し、行のどこを押しても選択)。詳細ポップアップの導線は名前spanへ移設 — `stopPropagation+showFighterPopup(id,'roster',true)`、従来顔に付いていた呼び出しと同一のため戻り先挙動は不変。
- **test/selection-surface-portrait-guard-test.js に§4新設**: 2行それぞれで「顔に第4引数なし」と「名前クリックのshowFighterPopupがある」を**対で**凍結(片方だけ消える退行を両方向で検出。「✕この選手を外す」行は`${c.name}`を含まないため検査対象から自然に除外)。ヘッダーの「対象外・裁定待ち」注記を裁定結果に置換。

検証: ガードテスト単体PASS+**npm test 245/245**(§4込み)+ui-baseline-guard PASS+ui-check7項目○(変更は既存24pxチップのクリック属性の付け替えのみ。新規カラー/吹き出し/コールバック待ちなし)。specs変更なし(選択サーフェス慣例の正はガードテストが背負う)。manifest変更なし(配布ファイル増減なし)。roadmap③行の「裁定待ちチップ」表記を解消し、実機確認バックログへ1項目追記。これで初回実走の同型6箇所は全クローズ。残: 実機確認(バックログ)。
