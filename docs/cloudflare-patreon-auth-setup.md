...
- **クリエイター本人のアカウントは自分のキャンペーンのメンバーではない** → **2026-07-24修正済み**: identityの所有キャンペーンが`PATREON_CAMPAIGN_ID`と一致すれば本人として自動通過する(campaignsスコープ追加)。実際に7日セッション失効後の再ログインで本人が弾かれる事故が起きたための恒久対応。`ADMIN_PASSWORD`(URLに`?password=…`)は引き続き非常口として有効
- **`PATREON_ALLOW_USER_IDS`**(任意・2026-07-24追加): カンマ区切りのPatreonユーザーIDを常に通す許可リスト。テスター招待や、campaignsスコープ不調時の保険。対象者のIDはログイン失敗画面に表示される
