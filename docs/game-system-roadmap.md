...
**v1.0e 完了** — ロスターランダム化実装済み。セッション8でtrainCap計算式確定＋成長モデル方針転換（イベント駆動型）。セッション9でv1.2の各種実装完了。セッション10で年末表彰式・殿堂入り・乱入マッチ・フレーバーイベントの詳細設計完了。
...
- **発生条件**：チャンピオンが3回以上防衛済み、かつ5年に4回の確率でランダム発生
...
- **発生対象**：チャンピオンまたは人気上位の選手にランダム発生
- **雑誌取材**：人気+微増、一定期間の好イベント発生率が少し上昇
- **TV出演**：ヒート+微増
- **演出**：業界誌・号外風テキストカードのポップアップ。選手の顔画像を添える

例：
> 📰 週刊女子プロレス
> [顔画像]「〇〇、インタビュー掲載。『まだまだ頂点を譲る気はない』」

> 📺 スポーツニュース出演
> [顔画像]「〇〇がゴールデンタイムに登場。業界への注目が高まっている」
...
- **乱入マッチ** — チャンピオン3回防衛後に5年4回ペースでランダム発生。隣接団体OVR90%以上の選手が乱入。勝利+2/敗北-15〜-20
...
| 8 | 乱入マッチイベント（設計完了） | 中 | 未着手 |
| 9 | フレーバーイベント・雑誌取材・TV出演（設計完了） | 中 | 未着手 |
...
| # | タスク | 重さ | 備考 |
|---|--------|:----:|------|
| 1 | **エース指定廃止・チャンピオン統合** | 中 | v1.2-6 |
| 2 | **チャンピオンの集客ボーナス** | 小 | v1.2-7 |
| 3 | **乱入マッチ実装** | 中 | v1.2-8。設計完了済み |
| 4 | **フレーバーイベント実装** | 中 | v1.2-9。設計完了済み |
