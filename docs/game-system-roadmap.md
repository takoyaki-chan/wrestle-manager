...
**v1.2 全完了** — ランキング・チャンピオンベルト改修9項目すべて実装済み。エース制度廃止→チャンピオン統合、集客ボーナス、乱入マッチ、フレーバーイベント完了。次はv1.3（選手ライフサイクル改修）へ。
...
| engine.js | ~3,520 | ゲームロジック全体（Engineオブジェクト＋initRandomRoster＋交渉＋乱入＋フレーバー） |
| app.js | ~2,620 | Audio+Storage+Mission+Survival+App統合+対抗戦観戦+タイトル画面制御 |
...
## 今セッション完了済み（セッション11 — v1.2-6〜9 実装）
...
| 2 | **個人実績記録システム新設（titleHistory含む）** | 大 | 未着手 |
| 3 | 引退演出（全選手：顔画像＋個性別セリフ） | 中 | 未着手 |
| 4 | 引退演出（格上げ対象：大演出＋固有セリフ） | 中 | 未着手 |
| 5 | 引退セリフ作成（98名分） | 中 | 未着手 |
| 6 | 殿堂入りシステム（設計完了） | 大 | 未着手 |
| 7 | 年末表彰式（設計完了） | 大 | 未着手 |

個人実績記録システム（`titleHistory`）で記録する項目:
- 通算試合数・通算勝利数
- タイトル取得週・喪失週・防衛回数（タイトルごと）
- 通算獲得回数・通算防衛回数
- 対抗戦・頂上決戦出場回数
- 在籍団体履歴・最長在籍シーズン数
- 年間最高OVR（全盛期記録用）
...
| # | タスク | 重さ | 備考 |
|---|--------|:----:|------|
| 1 | **個人実績記録システム新設（titleHistory）** | 大 | v1.3-2。殿堂入り・年末表彰の前提 |
| 2 | **衰退・引退テーブルの前倒し調整** | 小 | v1.3-1 |
| 3 | **引退演出（全選手・格上げ演出）** | 中 | v1.3-3/4 |
| 4 | **年末表彰式** | 大 | v1.3-7。設計完了済み |
