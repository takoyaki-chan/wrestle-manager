...
> 最終更新: 2026-02-26（セッション25 — 内部小数化＋MQボーナス体系見直し）
...
**セッション25: 内部小数化＋MQボーナス体系見直し。** 全リバランスの前提基盤として、popularity/orgPopの内部小数化を完了（表示はMath.round整数）。同時にMQ外部ボーナスに合計キャップ（+15）を導入し、因縁/タイトル/コーチ/観客の個別値も縮小。processShowを2パス方式に改修。「試合内容がMQの85〜90%を決め、外部要素は味付け」の原則を実現。
...
## 今セッション完了済み（セッション25 — 内部小数化＋MQボーナス体系見直し）
...
| `applyInjuryDecay()` | floor計算のMath.round除去 |
| `applyTransferReset()` | Math.round除去 |
| 自然減衰(-0.5/週) | 冗長な `Math.round(...*10)/10` 除去 |
| AI人気収束 | Math.round除去 |
| `applyOrgPopChange()` | 確率的丸め廃止 → `rawDelta * mult` を小数で返す |
| UI 14箇所 | ui-render.js 8箇所・ui-common.js 6箇所を `dispPop`/`dispOrgPop` で整数表示に統一 |
| app.js | マイグレーション内Math.round除去、orgPopログを小数1桁（+0.4形式）に |
...
| コーチ（紅林） | +3 | **+2** |
| 超満員/大入り観客 | +5/+3 | **+3/+2** |
| 会場規模MQ | 0,0,1,1,2,2,3 | **0,0,0,1,1,1,2** |
| MQ_EXTERNAL_CAP | (なし) | **+15（新設）** |

processShow 2パス方式:
- Pass 1: バトル結果生成 → 因縁・タイトル・コーチのメタデータ記録（MQ加算なし）→ 因縁更新 → タイトル処理
- Pass 2: 全外部ボーナスを合算し、正方向をキャップ（+15）適用 → r.mq に加算。ガラガラ(-3)ペナルティはキャップ対象外
...
| 4 | **弱い相手とのタイトルマッチにマイナス要素** | 中 | OVR差が大きい場合のペナルティ |
| 5 | **ランキング画面のカスタムツールチップ** | 小 | モバイル対応含む |
| 6 | **特性未実装4種の実装**（適応力/人望/忠誠心/野心） | 中 | セッション19の照合で特定済み |
| 7 | **全体的なUI/UX改善** | 中〜大 | 具体的な項目は次セッション冒頭で洗い出し |
