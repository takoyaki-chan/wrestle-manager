...
> 最終更新: 2026-03-07（ライフサイクルリデザイン Phase 4 実装完了）
...
| Step | タスク | 重さ | 状態 |
|------|--------|:----:|------|
| 1 | **定数追加** data.jsに`BIGMATCH_MAX_T`(24), `BIGMATCH_PHASES`(6T×4phase), `BIGMATCH_ENG`(hpBase:85等) | 小 | 未着手 |
| 2 | **エンジン分岐** simulateMatchに`matchTier`引数追加、Tier別の定数切り替え（ロジック変更ゼロ） | 中 | 未着手 |
| 3 | **MQ対応** ペーシング減点の適正ターン帯をTier分岐（Tier2: 13-21が適正） | 小 | 未着手 |
| 4 | **呼び出し側更新** 全simulateMatch呼び出しにmatchTier渡し。PPV/タイトル/トーナメント/対抗戦→Tier 2 | 中 | 未着手 |
| 5 | **BGMスロット** BGM_SLOTS定義、bigmatch_intro/match/climax/finish/resultの5スロット、null時フォールバック | 小 | 未着手 |
| 6 | **演出: ライバリー台詞カットイン** 試合ログ表示時に因縁/宿敵/宿命の相手で30〜80%確率発動。性格×アーキタイプの台詞プール | 中 | 未着手 |
| 7 | **演出: ブレイクスルー兆し** BT成功時に遡って試合中にモノローグカットイン挿入 | 中 | 未着手 |
| 8 | **観戦モード対応** battle-engine.htmlにTier 2用フェーズ表示・BGM切り替え・カットインUI | 中 | 未着手 |
