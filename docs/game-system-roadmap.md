...
- **宣戦布告ポップアップ刷新**: upper画像を縦210px大型表示・右キャラをscaleX(-1)でミラー向かい合わせ。吹き出しを2カラム横並び、スピーカー名ラベル付き。ヘッダーBebas Neue大型表示。タイミングを「全試合まとめて前置き」→「その試合のカードがフォーカスされた瞬間に自動表示」に変更
- **レンタル残週数バグ修正**: `seasonsLeft×12`固定値→`(seasonsLeft-1)×12+(13-currentWeek)`の実残週数計算に修正。週ごとに1ずつ減るように
- **今週画面のレンタル枠分離**: 今週テーブルで自団体選手とレンタル選手を`🤝 レンタル枠 (N/M)`セパレータ行で分離表示
- **タイトル戦BGMバグ修正**: `escapeBattle()`でbigmatch BGMが停止しなかった問題を修正（`Audio.fileBgm.stop()`追加）
...
- **第1層イベント（双方条件4象限）**: 親友（condition保護/スランプ支援）、憎い敵（衝突→次戦MQボーナス/成長↑+怪我↑のハイリスク・ハイリターン）、好敵手的（切磋琢磨小ボーナス/MQ二重取り）、無関心（何も起きない）
- **第2層イベント（非対称条件）**: 片思い（bond差30+）、一方的な敵意（rivalry差30+）、温度差（bond差15〜29）、クロス非対称（rivalry×bond交差の切なさ/覚醒イベント）
...
- **E2追加修正: セリフ性格分類の統一（2026-03-13）** — キャラクター直接発話セリフの全箇所をpersonality×archetype辞書形式に統一。修正1-3: RIVALRY_CONFRONTATION_LINES/RIVALRY_CONFRONTATION_LINES_70/RIVALRY_CONFRONTATION_LINES_90をpairs配列→attacker/defender(+fateAttacker/fateDefender)のpersonality×archetype辞書に変換。修正4: UPSET_RIVALRY_LINES.loserLines新規追加（番狂わせ敗者専用セリフ、格上が格下に負けた衝撃・動揺）。修正5: WEEKLY_STORY_TICKER.awakeningをフラット配列→personality×archetype辞書に変換（覚醒するnameB側のpersonalityで選出）。修正6: FAN_EXPECT_REACTIONS.goodWinner/badWinnerをフラット配列→personality×archetype辞書に変換（勝者のpersonalityで選出）。UI: ui-common.js宣戦布告ポップアップをpickDialogueLine(attackerPool,leftFighter)/pickDialogueLine(defenderPool,rightFighter)に変更。番狂わせ時loserLinesをUPSET_RIVALRY_LINES.loserLinesから選出。engine.js覚醒イベントをgetDialoguePool+Engine.rng管理の選出に変更。app.jsファン期待カードリアクションをpickDialogueLine化。全7personality×6archetype対応。auto-sim 500シーズンALL CLEAR
...
- 2026-03-16: 序盤orgPop保護 — getMQAdjustにearlyProtectシフト追加。orgPop < 30の弱小団体はファン期待値が低く、低MQでもorgPopペナルティが入りにくい。earlyProtect = -round((30 - orgPop) * 0.4)。orgPop=10で-8、orgPop=20で-4、orgPop=30+で変更なし。getPopMQShift/getTrustMQShiftは対象外。auto-sim 100シーズンALL CLEAR。
