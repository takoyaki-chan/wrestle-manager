...
> 最終更新: 2026-03-21（収支チャート全サブタブ対応 実装完了）
...
**収支チャート全サブタブ対応 実装完了（2026-03-21）。** 収支画面の4サブタブ（総合・収入・支出・給与）すべてにSVG折れ線チャートを追加。`_financeChart()`共通関数化+`_weeklyFinanceValues()`週次集計ヘルパー新設。期間フィルタ（今月/年間/全期間）連動。UIのみの変更。
