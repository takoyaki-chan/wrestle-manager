...
> 最終更新: 2026-02-26（セッション24 — 難易度リバランス設計＋第1弾実装）
...
**セッション24: 難易度リバランス設計＋第1弾実装。** プレイヤーのスクリーンショットから「3-4年でorgPop100/Heat MAX/資金ジャブジャブ」の問題を特定。現行コードの定量分析を行い、5施策の設計書（difficulty-rebalance-design-v1.0.md）を作成。第1弾として施策A（orgPop逓減カーブ）＋B-1（年次減衰強化）＋E（Heat維持困難化）を実装完了。内部小数化・MQボーナス体系見直しの方針を確定。
...
- **人気・orgPop・Heatの内部小数化（予定）** — 表示は整数（切り捨て）、内部は小数第2位まで保持。整数丸めだと減衰カーブが高帯域で機能しなくなる問題（+0.45が毎回0に切り捨て→永遠に上がらない or 確率的丸めの不自然さ）を根本解決。popularity/orgPop/heatScoreの全3値が対象
- **MQボーナス体系の見直し（予定）** — 現行は外部ボーナス（因縁+22/タイトル+15/観客+8/コーチ+3/特性+13）が最大+45に達し、試合内容が凡戦でもMQ100になる異常構造。「試合内容がMQの85〜90%、外部要素は味付け」の原則で全数値を再設計。外部ボーナス合計に上限（+15程度）を設ける。内部小数化が前提
- **難易度リバランス第1弾（セッション24）** — orgPop逓減カーブ導入（Engine.orgPop名前空間新設。6段階: ×1.0/×0.60/×0.35/×0.20/×0.12/×0.08。確率的丸めで高帯域もゆっくり上昇）。年次減衰をorgPop比例化（固定-3→帯域別-2/-3/-5/-7/-10）。Heat維持困難化（HOT以上で上昇×0.5＋冷め速度1.5倍）
...
## 今セッション完了済み（セッション24 — 難易度リバランス設計＋第1弾実装）
...
| # | タスク | 重さ | 状態 |
|---|--------|:----:|------|
| 1 | **内部小数化**（popularity / orgPop / MQ関連ボーナス） | 中 | 次着手。全リバランスの前提基盤 |
| 2 | **難易度リバランス第1弾**（orgPop逓減＋年次減衰強化＋Heat維持困難化） | 大 | ✅ 完了 |
| 3 | **MQボーナス体系の見直し** | 大 | 内部小数化後。外部ボーナス合計に上限、全数値を小数ベースで再設計 |
