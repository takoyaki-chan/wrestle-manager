...
> 最終更新: 2026-04-22（派閥システム Phase 3c 相関図派閥ビューモード 実装完了）
...
**派閥システム Phase 3c — 相関図 第4ビューモード「🎭 派閥」実装（2026-04-22）。** `specs/faction-system-spec-v0.1.md` Phase 3 の 3 つ目（3c）、spec v0.6。ネットワーク/フォーカス/勢力図に続く第4ビューモードとして派閥専用表示を新設。①`src/ui-render.js` `_relmapViewMode` に `'faction'` 追加、新規状態 `_relmapFactionCenters: { factionId: {x,y} }`。②`.rm-view-toggle` に 🎭 派閥ボタン（4番目）追加、`G.factions.length === 0` で disabled + title「派閥が結成されていません」。③`_relmapComputeFactionCenters()` 新設: 派閥数に応じた等分配置（N=1: 中央、N=2: `W*0.3/W*0.7` 左右、N=3: 三角 R=min(W,H)*0.28、N=4+: 円周 R=min(W,H)*0.32、`-π/2` 起点時計回り）。④`_relmapTick` 先頭に `if (_relmapViewMode === 'faction')` 別枝追加: 派閥メンバーは派閥中心へ引力 0.015、非メンバーは画面中央へ弱い引力 0.004、Link attraction / Repulsion は network と同パラメータ流用（既存 3 モード分岐は無変更）。⑤`_relmapSetViewMode('faction')` 分岐追加: SVG レイヤー再構築（power モードからの戻りに備え defs + zoneLayer/factionLayer/linkLayer/nodeLayer 再構築）+ `_relmapOrgFilter = null` + ズーム/パンリセット + 全ノード unhide + 団体ゾーン非表示（派閥円と重なるため）+ `_relmapComputeFactionCenters()` 呼び出し + `_relmapRenderSidebar` + `_relmapReheat()`。btnIdx 計算を `mode === 'power' ? 3 : 4` に修正。⑥`_relmapDrawFactionLayer()` 本実装（no-op 廃止）: 派閥モード時のみ描画、(1) 抗争破線（`_isHostile(A) && _isHostile(B)` かつ `hostility[A>B]+hostility[B>A] > 0` のペア、太さ 1.5〜3.5px を合計 0〜200 で線形補間、stroke-dasharray="6,4"、`--accent-faction-feud`、opacity 0.7、**数値ラベルなし**spec §7.4 厳守）、(2) 派閥外接円（メンバーノード位置の重心→最遠距離+24px を半径、単独時 r=48px 固定、`--accent-faction-{1..4}` を `(idx % 4) + 1` で割当、stroke-width 2.5 / stroke-opacity 0.55、fill="none"）、(3) 派閥名ラベル（円の上辺外側 `y = cy - r - 12`、text-anchor="middle"、font-family="Oswald,sans-serif" 16px 700、派閥色 opacity 0.85、HTML エスケープ済み）、(4) リーダー 👑 14px / 幹部 ⭐ 12px（OVR 上位 2 名、`Engine.factions.isExecutive` と同ロジック）をノード右上 `(x+r*0.6, y-r*0.6)` に重ね描画。OVR マップは G.roster/freeAgents/rivalOrgs.roster から構築。⑦`_relmapResetAll` の network 復帰分岐に `|| _relmapViewMode === 'faction'` を追加。⑧Keisuke 推奨案採用記録（spec §17）: 画角=ばね定数強め/非派閥メンバー=中央弱引力/抗争相手なし loyal 派閥=円のみ/モード切替=reheat のみ。触っていない領域: `_relmapTick` のネットワーク/フォーカス/勢力図 3 分岐の既存パラメータ、`_relmapRender` 既存描画ループ、`_isHostile` 判定、`G.factions`/`factionHostility`/`factionEventCooldowns` データ形状、F02 再設計方針、Phase 2/3a/3b の派閥 UI、`calcMatchAppeal` factionAppeal 分岐。検証: auto-sim 50 シーズン ALL CLEAR（UI のみ変更のため）。UI 実機確認は Keisuke さんに委任。Phase 3d（bond/rivalry 連動カタログ）で派閥システム v1.0 完成予定。変更: src/ui-render.js(_relmapViewMode+_relmapFactionCenters/派閥ボタン追加/_relmapSetViewMode faction分岐/_relmapComputeFactionCenters新設/_relmapTick faction重力項/_relmapDrawFactionLayer本実装/_relmapResetAll faction対応)+specs/faction-system-spec-v0.1.md(§17 Phase 3c完了記録+変更履歴 v0.6)+plans/faction-phase3c-task.md→plans/archive/(アーカイブ予定)。
