...
**セッション31完了。** バグ修正1件 + UX改善2件 + 序盤〜中盤の難易度崖緩和3件。

- **Bug修正 A-1**: ケアモーダルCSS変数不整合 — `:root` に `--bg-card`/`--bg-main`/`--border` エイリアスを追加。モーダル背景が透明になる問題を解消
- **UX改善 A-2**: ケアアクション確認ダイアログ追加 — 団体向けアクション（打ち上げ/合宿）に `renderTeamConfirm()` による確認画面を挿入。誤操作防止
- **UX改善 A-3**: おまかせボタン改善 — `autoManage()` を「設定のみ」に変更（processWeek自動実行を廃止）。興行週でもボタン表示。title説明文更新
- **難易度調整 B-1**: `getMQAdjust` 保護帯域を4段階化（旧3段階）— orgPop 30〜44 の橋渡し帯域（shift:-3/negMult:0.85）を新設。20〜29帯の保護も強化（shift:-7/negMult:0.5）
- **難易度調整 B-2**: orgPop 20〜39 の逓減カーブ緩和 — ×0.60 → ×0.70（約+17%成長速度）
- **難易度調整 B-3**: 育成補助金 orgPop 20〜29 帯を緩和 — 50万/週 → 65万/週
...
- **育成補助金** — orgPop 40未満に地域振興助成金（0-19:80万/週、20-29:50万/週、30-39:20万/週）
- **orgPop逓減カーブ** — 0→×1.0, 20→×0.60, 40→×0.35, 55→×0.20, 70→×0.12, 85→×0.08
- **orgPop帯別MQ閾値シフト** — orgPop<20:shift=-10/negMult=0.4、orgPop<40:shift=-5/negMult=0.7、40以上:変更なし（Engine.orgPop.getMQAdjust）
- **ケアシステム週次制限** — costume/mediaは週1回/選手/アクション（_careWeekUsed[actionId]===week で管理）
