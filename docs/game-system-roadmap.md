...
| タッグマッチ・タッグ王座 | 中 | **Phase 1+2+3+4a完了（2026-04-17）**: Phase1:Engine統合。Phase3:結果処理統合(executeShow全面対応)。Phase2:興行カード編成UI。Phase4a:ビジュアル観戦(Replay方式、tag-battle.html新規)。**【方針転換 2026-04-17】今後のタッグ演出・タイミング・UI はすべて "シングル battle-engine.html を基準" に合わせる**。タッグ Replay 側でピン/タップ/カウント演出を独自実装した結果、シングルで既に解決済みの問題を再発明する悪循環が発生した(今日までに7コミット以上要した)。**Phase 4b (優先度↑)**: シングルを "4人対応+Replay化" に拡張してタッグと共通基盤化する統合リファクタ。これ以降のタッグ改修は先にシングル側で直し、共通コードに落とすルートに統一。**残タスク(別日、Phase 4b 完了後に着手推奨)**: T1:試合が50ターン級に長引く問題の再調整 / T3:ダブルチーム演出のテキスト詳述(narration で何をやっているか分かるように) / T4:試合結果画面で各選手の一言台詞表示 |
