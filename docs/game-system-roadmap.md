...
> 最終更新: 2026-03-07（Phase 6: 人物相関図 UI）
...
**Phase 6: 人物相関図 UI 実装完了（2026-03-07）。** データベース画面のサブタブ「🔗 相関図」として人間関係の放射型可視化UIを実装。選手詳細ポップアップからの「🔗 RELATIONSHIP MAP」ボタン導線、キャラ選択セレクトボックス、4種フィルタ（全体/同団体/ライバル/親密度順）、SVG接続線（非対称bond矢印+rivalry破線オーバーレイ）、関係リスト＋インフォバーを完備。UI層のみの変更でエンジン影響なし。
