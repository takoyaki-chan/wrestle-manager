...
| 日付 | 内容 |
|------|------|
| 04-15 | 団体年代記 Phase 3 (chronicle-system-spec-v0.1.md §9 Phase 3 — 細部調整): 2枚看板専用レイアウト: `.chron-dual-wrap` (grid 1fr/28px/1fr) + `.chron-dual-card` (縦積み、portrait 130×168px + 個別タグライン/名前/4統計/記者コメント) + `.chron-dual-divider` (中央縦線 + `＆` マーク)。HoF 相互リンク: エース・同期に `🏅` バッジ (`.chron-hof-badge`) + `.chron-hof-link` (クリックで `openHofDetailById(id)`)。HoF 詳細モーダルに「📖 年代記で見る」ボタン (`hasChronicleChapter` 判定、`openChronicleForFighter(id)` で _dbSubTab=6 + _dbChronicleIdx=N + renderDatabase())。手動再構築ボタンは Phase 1 実装済み確認。UI 層のみ変更のため engine auto-sim 不要。変更: ui-render.js (_renderDbChronicle エースセクション全面改修 + 同期リスト HoF バッジ追加 + CSS 追加 + openHofDetailById/openChronicleForFighter 新設 + showHofDetail hasChronicleChapter ボタン) |
