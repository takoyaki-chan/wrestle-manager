...
| 変更 | 詳細 |
|------|------|
| `Engine.career.generateDurability` | 正規分布μ=0,σ=2で個人耐久値生成（-4〜+4） |
| `makeChar` 拡張 | `durability` / `wear: 0` 追加 |
| `applyDecay` 置き換え | wear閾値ベースの全ステ減少（20-39: -1〜2、40-59: -2〜4、60-79: -3〜5） |
| `applySeasonEnd` 拡張 | 28+dur歳からwear蓄積（baseWear 7〜13 + 試合数/怪我数/intensive補正） |
| `checkRetirement` 置き換え | wear80+確定、60+50%、40+20%、自主引退は維持 |
| AI選手対応 | `processSeasonEnd` にwear蓄積ロジック追加 |
| 引退チェック拡張 | 重傷時 §4.2 wear超過引退 / §4.3 壊滅的怪我（2〜3%/5〜8%）を `injury.check` に追加 |
| マイグレーション | `_migrated_v1_3_1`：既存セーブに durability/wear 後付け |
| UI | ロスター・選手詳細に wear ラベル（⚠ 衰え / ⬇ 衰退期 / ⬇⬇ 限界） |

### 成長システム改訂・怪我デバフ（specs/v1.3-2-growth-injury-spec.md）

| 変更 | 詳細 |
|------|------|
| `seasonInjuries` | 怪我発生ごとにカウント。シーズン末リセット。wear += seasonInjuries×2 に反映 |
| `careerHistory[]` | 中傷/重傷・引退を時系列記録。選手詳細TAB1末尾に表示 |
| `growthPenalty` | 怪我発生時付与（軽傷:6週×0.7 / 中傷:14週×0.4 / 重傷:24週×0.15）。毎週カウントダウン |
| 練習成長×0.4 | `calcGrowth()` の結果に `× 0.4 × penMult` を適用（intensive/practice両方） |
| 試合成長 | matchGrowthBase=1.5 + 対戦相手補正 + 接戦補正(MQ≥65) + 負け補正 → 1〜2ステに分散 |
| マイグレーション | `_migrated_v1_3_2`：既存セーブに seasonInjuries/careerHistory/growthPenalty 後付け |
| UI | ロスターに🩹バッジ、選手詳細にgrowthPenalty残週・careerHistoryセクション |

---

## 前セッション完了済み（セッション12 — v1.3-2 careerRecord 実装）
...
| # | タスク | 重さ | 状態 |
|---|--------|:----:|------|
| 1 | **衰退・引退システム改修（durability + wear方式）** | 大 | ✅ 完了 |
| 2 | **個人実績記録システム新設（careerRecord + retiredFighters + hallOfFame）** | 大 | ✅ 完了 |
| 2b | **成長システム改訂・怪我デバフ（試合成長/growthPenalty/careerHistory）** | 大 | ✅ 完了 |
