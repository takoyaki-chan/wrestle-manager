...
**セッション30完了。** バグ修正2件 + 序盤難易度調整。

- **Bug修正**: 月末再試合バグ — `App.executeShow()` に weekPhase ガード + `renderShowPrep()` を settled/weekSummary フェーズで非表示化
- **Bug修正**: ケアシステム週次制限 — `costume`/`media` を週1回/選手/アクションに制限（`_careWeekUsed` 追跡）
- **難易度調整**: orgPop帯別MQ閾値シフト（`Engine.orgPop.getMQAdjust()`）— 低orgPopほどMQ閾値が下がり上がりやすく・下落ペナルティも軽減。旧フラット+0.2ボーナスを廃止してこちらに統合

セッション29完了分: タイトルマッチ格差ペナルティ（OVR差>10:MQ-3、>20:MQ-6）＋特性4種実装（適応力/人望/忠誠心/野心）。v2.0イベントシステム Phase 1（Step 1〜5）完了。
