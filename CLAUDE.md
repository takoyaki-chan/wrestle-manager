...
### 4. 完了前の検証
- **UI確認はスクリーンショットではなくユーザーに委任する。** preview_screenshot は時間コストが高いため使用しない。ロジックの正当性はコンソール eval・エラーチェックで担保すること。
- **実装完了後は即座に完了報告し、目視確認ポイントを列挙する。** ユーザーの事前承認を待たず実装を完了してよい。報告時に「確認してほしい画面・操作・表示」を具体的に伝え、ユーザーが肉眼で検証する。問題があれば追って修正する。
