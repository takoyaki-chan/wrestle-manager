...
### ダメージセリフ/ボイスのフェーズ別発動ルール
クリティカルヒット（dmg≥15）時のみ発動判定。フェーズによって使い分ける:
- Opening: なし（余裕がある）
- Mid: セリフ40%（まだ言葉にできる）
- End: セリフ15% / ボイス50%（言葉が出にくくなる）
- Climax: ボイス60%のみ（限界、言葉にならない）
ビッグムーブも同じルール。Climaxでダメージセリフ（長文）は絶対に出さない。
