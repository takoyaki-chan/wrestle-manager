...
> **プレイヤー向け表記(2026-08-31確定)**: 「MQ」は内部名であり、プレイヤーに見せる画面・ログ・新聞では**「試合評価」**(短縮ラベルは「評価」、複合語は「平均評価/最高評価/通算評価/決勝評価」)と表記する。旧セーブに焼き込まれた旧表記はロード時マイグレーション(`_migrated_mq_text_v2`, src/app.js)で同表記へ書き換える(初版v1はcurrentNewspaper/mvpRaceのキー漏れ+和文直結型の未カバーがあり2026-08-31にフラグをv2へ昇格して全文再走・冪等。回帰網は test/mq-text-migration-v2-test.js)。走破検出器(test/ui-walkthrough/detectors.js)が可視テキスト中のMQトークンを恒常検査する。本spec内の「MQ」は内部システム名としての使用であり変更しない。
