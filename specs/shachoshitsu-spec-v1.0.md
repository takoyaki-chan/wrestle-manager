...
- **カテゴリ**: `care`
- **コスト**: 単価15万×人数 / 決裁枠1
- **発動条件**: `morale_low` — ロッカールームmoraleが50未満
- **効果対象**: 団体全員
- **効果**: 全員の信頼度微上昇 + ロッカールームmorale上昇
- **cooldown**: 1週（団体全体で1回）
- **書類見出し**: 「慰労会開催届」
...
| `morale_low` 🔧 | `(state.lockerRoomMorale ?? 60) < 50` |

**閾値の根拠**:
- **trust < 60**: 信頼度60を「安定」の基準とする。60以上は介入不要、60未満で要注意。60は既存のtrustシステムで「high帯(70+) / normal帯(40-69)」の境目に近い値。
- **morale < 50**: 雰囲気が悪化した状態。デフォルトmorale=60なので、10以上下がった状態を「悪化」とみなす。
...
クリック可能状態と無効化状態:
- **クリック可能**: 通常表示 + ホバー有効
- **資金不足**: 書類がわずかに暗くなる + ホバー無効 + ツールチップ「資金不足」
- **決裁枠不足**: 書類がわずかに暗くなる + ホバー無効 + ツールチップ「決裁枠不足」
- **決裁済み**: 朱印付き画像 + ホバー無効
