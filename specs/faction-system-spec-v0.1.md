...
**演出**: 中量版。不満分子の陰口・リーダーへの不満の声から開始。BGM は `Soft Bids, Sharp Minds.mp3` を `SOFT_VOL × 0.14` 相当で穏やかに敷く（内輪揉めのトーンを壊さない）。
...
**演出**: 軽量〜中量版。両派閥メンバーが何気ない雑談を交わすシーンから開始。BGM は `Soft Bids, Sharp Minds.mp3` を `SOFT_VOL × 0.16` 相当で明るめに敷き、結果モーダル「閉じる」時に `f06_fin_chime_v1.mp3` を `CHIME_VOL × 0.10` で1打（「終わりの合図」として和解を締める）。
...
**演出**: 中量版〜やや重め。練習場での視線の衝突、廊下ですれ違うシーン。BGM は `bgm_tension_v1.mp3` を `TENSION_BED × 0.17` 相当でループ、冒頭 stinger `f07_gong_v1.mp3` を `GONG_VOL × 0.15` で 150ms 遅延の 1 打（F02/ignite 系との統一感、「対立が始まる」予感を音に乗せる）。
