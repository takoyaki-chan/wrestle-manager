...
1. **growthBonusの扱い** — 調査の結果、未参照の死に定数と判明(§4)。論点は「(a) 削除して現状追認(推奨・挙動変化なし)」か「(b) 対称原則の例外としてあえて新規実装するか」。(b)を選ぶ理由は現状見当たらない
2. **P-5の付与確率**(S 50% / elevated A 30%)のみ — 対象選出「trainCap上位3からランダム」と候補フィルタ「衰えが少しでも見えたら候補外」は確定済み(2026-08-01)
