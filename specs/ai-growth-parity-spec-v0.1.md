...
- AIの週次行動モデルは「**毎週おまかせ(balance)を押す社長**」(2026-08-01 Keisuke確認)。プレイヤーの balance 分岐と同じ判定規則を適用し、可能な限り同じ判定コードを共有して将来のドリフトを防ぐ。プレイヤーの上振れ余地は おまかせの外側(追い込みのタイミング/コーチ選定/社長室ケア/マッチメイク)に残る
- タレント活動(B4イベント)は既にAI団体も同ロジックで処理されており(9037)、本仕様の対象外(既に対称)
...
- Keisuke裁定「限定的に」: **1団体1名のみ・シーズン単位の確率付与・常設しない**
- **対象選出(2026-08-01 Keisuke裁定)**: `trainCapOVR`(Engine.rival.trainCapOVR 8100)**上位3名の中からランダムで1名**。現在OVRの番付ではなく伸びしろに投資する — AIが有望株を育てる挙動になり、シーズンごとに違う選手が伸びてくる
- シーズン開幕処理で付与判定:
  - S級: 50% 🔧 で発動し、上記の方法で1名選出
  - A級: leagueElevated 時のみ 30% 🔧、同様に選出
  - B級: なし
- **候補フィルタ(2026-08-01 Keisuke裁定・確定)**: **少しでも衰えが見えた選手は候補外**。判定は「`wear = 0` かつ 衰えによるステ減少(statPeak比の▼)がゼロ」— どちらか一方でも欠けたら対象外。衰退期(decayStartAge)未到達でも初期wearを持つ選手は候補外になる。フィルタ後の候補が3名未満なら残った候補から選出、0名ならその団体はそのシーズン付与なし
- 効果・期間はプレイヤーの外部コーチ招聘(shachoshitsu-care-rework §3)と**同値を参照**する。AI専用の倍率を新設しない
- 同時付与は業界全体で最大2名(S + elevated A)

### P-6 死に定数の整理(coachMul / growthBonus)

- AI_COACH_CONFIG の `coachMul` は週次成長経路で未使用(§1.1注)。8374 の参照用途を実装時に特定し、死んでいれば削除、生きていれば用途をコメントで明記
- AI_TIER_LIMITS / AI_TIER_LIMITS_ELEVATED の `growthBonus` は**参照ゼロの死に定数**(2026-08-01 grep確認)。削除する
...
- **growthBonus(A ×1.10 / B ×1.08、normal側の S ×1.12 / A ×1.05 も同様)は現行ビルドで未参照の死に定数**(2026-08-01 grep確認: data.js 8535-8537 / 8634-8635 の定義のみで適用箇所ゼロ。旧aiSeasonGrowth時代の遺物)。**削除して現状追認する**(P-6の整理に統合)。つまり現行の覚醒イベントも既にこの乗算なしで動いており、削除による挙動変化はない。覚醒の強さは実在する要素(practiceRate増/エース枠増/コーチ格上げ/FA・ドラフト積極化/P-5トレーナー)で表現する
- S級は現行どおり変更なし
...
## §9 レビュー決着(2026-08-02 Keisuke承認)

1. **growthBonus** — 未参照の死に定数と判明(§4)。**削除して現状追認で確定**(挙動変化なし)
2. **P-5の付与確率** — **初期値(S 50% / elevated A 30%)のまま計測へ進むで確定**。対象選出「trainCap上位3からランダム」/候補フィルタ「衰えが少しでも見えたら候補外」は2026-08-01に確定済み
3. **下位層の底上がり** — **計測後判断で確定**(§6の結果を見て、必要なら下位枠のpracticeRateで絞る。rateは采配=世界観の表現であり、下げても公平原則には反しない)
4. **開眼(化ける)システムとの接続** — 本spec較正後のトップ実到達帯(G1)を開眼の着地帯の物差しにする。開眼specは本specの計測完了後に起票する(変更なし)
