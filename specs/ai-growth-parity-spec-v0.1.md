...
- **growthPenalty(ペナルティ期)**: AI側に該当stateが発生しないため対象外
  (→ **2026-08-29訂正**: 引き抜き・移籍でプレイヤー団体から持ち込まれるため発生しうる。§1.5参照)

### §1.5 【2026-08-29追補】状態tickの非対称(§1調査の見落とし・Fix-A済み)

OVR100調査(docs/growth-ovr100-investigation-v0.1.md)で、§1の全量調査から漏れていた非対称が発覚:

| # | 内容 | 影響 |
|---|------|------|
| 6 | **スランプ/モチベ喪失の回復判定(tickSlumpPassive/tickMotivationLossPassive)がプレイヤーロスター週次にしか無い**。AI側は発生(敗北時)とmomentum加算だけで回復のダイスを一度も振らない | AI所属選手は一度スランプに入ると永久凍結(練習・試合成長とも×0)。AI高pot若手の37〜40%が該当、凍結組はmeanPeak -9.5pt |
| 7 | 絶好調(hotStreak)のカウントダウンがAI側に無い | AIの絶好調が永続(成長×1.15が恒久化 = AI有利側の非対称) |
| 8 | growthPenaltyのカウントダウンがAI側に無い | 引き抜きで持ち込まれた選手のペナルティが永続 |
| 9 | スランプ/モチベ喪失中の能力微減(applyWeeklyStatDecay)がAI側に無い | 状態異常の代償を払わない(AI有利側) |

**是正(2026-08-29実装)**: processAIWeek のロスターmap先頭に、プレイヤー週次(management.js 12500帯)と
同順序・同怪我ガードの状態tick一式を追加。乱数は選手ごと `create(derive(seed, 季, 週, ソルト, id))`
(ソルト0xA5C2〜0xA5C6)。**意図した非対称として残した2点**:
①コーチ系倍率(スランプ回復mult等)はAIに存在しないため中立1.0
②モチベ喪失の自主引退(§5.5)はAI側ではロスター除去に踏み込まず「その週は回復なし」に読み替える
(王座・カード等への波及が大きいため。将来AI中途退団を設計する際の宿題)
...
- **→ 2026-08-02 再実行完了**: 切り離しプロセスで seed 1001 × 100シーズンを完走(5300週・7.6分)。Game overs 0・**ALL CLEAR(エラー・不変条件違反 0)**。§6の計測項目はすべて消化済み。
