...
| i18n-runtime-spec-v1.0.md | **i18n実行基盤(Stage A確定分)**: WM_I18N.t()/wm_lang/data-i18n/構造規約7条(Engine非依存・断片連結禁止・gameLog type形式等)/常設ガード(JAゴールデン・ラチェット・擬似ロケール) |
...
| spring-tag-league-spec-v0.2.md | 春のタッグリーグ **2ブロック制への作り直し**(4チーム×2ブロック+ブロック1位同士の決勝/出場枠は団体順位で3-2-2-1の計8チーム/賞金・引き分け裁定を改訂、2026-08-01 起票・**task-96で実装完了 2026-08-13**。§10未決2点はtask-96指示書のFable裁定が正: AI2組目以降=未選出OVR上位ペア/蛇行固定。対戦ポイントは各団体1回=v0.1踏襲) |
